...
(function(){
  ...
  function ensure(group, callback){
    ...
    if(!gate||typeof gate.ensureGroup!=='function'){callback();return;}
    gate.ensureGroup(group).then(function(ready){if(ready!==false)callback();}).catch(function(e){console.error('PETATOE group readiness error',group,e);});
  }
