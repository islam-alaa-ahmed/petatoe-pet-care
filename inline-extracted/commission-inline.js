...
function renderArchive(){const snaps=readSnaps();const keys=Object.keys(snaps).sort().reverse();return `<div class="com-card"><h3>📚 أرشيف العمولات الشهرية</h3><p>كل شهر مقفل يحتفظ بالأرقام والإعدادات كما كانت وقت القفل.</p><div class="com-table"><table><thead><tr><th>الشهر</th><th>السيارات</th><th>Groomers</th><th>Drivers</th><th>Sales</th><th>الإجمالي</th><th>تاريخ الحفظ</th></tr></thead><tbody>${keys.map(k=>{const s=snaps[k];return `<tr><td>${k}</td><td>${fmt0(s.cars||0)}</td><td>${petBlock5568_money(s.groomerTotal||0)}</td><td>${petBlock5568_money(s.driverTotal||0)}</td><td>${petBlock5568_money(s.salesTotal||0)}</td><td>${petBlock5568_money(s.total||0)}</td><td>${safe(s.savedAt||'')}</td></tr>`}).join('')||'<tr><td colspan="7">لا توجد شهور مقفلة بعد.</td></tr>'}</tbody></table></div></div>`}
