...
function dataRows(){
  try{
    if(window.PETATOERecordsReadFacade&&typeof window.PETATOERecordsReadFacade.readRows==='function') return window.PETATOERecordsReadFacade.readRows()||[];
    if(window.PETATOERecordsReadFacade&&typeof window.PETATOERecordsReadFacade.getRecordsSync==='function') return window.PETATOERecordsReadFacade.getRecordsSync()||[];
    return window.PETATOEDataSource&&typeof window.PETATOEDataSource.getRecordsSync==='function' ? (window.PETATOEDataSource.getRecordsSync()||[]) : [];
  }catch(e){return []}
}
...
function buildCalcForPeriod(p, carFilter, options){
  ...
  const snap=options.ignoreSnapshot?null:readSnaps()[p];
...
async function commissionCertifySnapshot(calc,previous,replacementReason){
  ...
  snap.financialAudit={invoiceCount:Number((snap.traceability.summary&&snap.traceability.summary.invoiceCount)||0),lineCount:Number((snap.traceability.summary&&snap.traceability.summary.lineCount)||0),eligibleLineCount:Number((snap.eligibilitySummary&&snap.eligibilitySummary.eligibleRows)||0),excludedLineCount:Number((snap.eligibilitySummary&&snap.eligibilitySummary.excludedRows)||0),eligibleNet:Number((snap.eligibilitySummary&&(snap.eligibilitySummary.eligibleAmount!==undefined?snap.eligibilitySummary.eligibleAmount:snap.eligibilitySummary.eligibleNet))||0),commissionTotal:Number(snap.total||0)};
...
window.PETATOECommissionCalculationUAT={version:'1.0.0',owner:'inline-extracted/commission-inline.js',paymentPolicy:'accrual_all_eligible_invoices',rowNetSales:rowNetSales,classifyRow:classifyCommissionRow,eligibilitySummary:commissionEligibilitySummary,segmentFor:segmentFor,calculateLive:function(period,carFilter){return buildCalcForPeriod(period,carFilter||'',{ignoreSnapshot:true});},calculateDisplayed:function(period,carFilter){return buildCalcForPeriod(period,carFilter||'',{});},reproduceSnapshot:commissionReproduceSnapshot};
...
function commissionAllowedCars(){
  ...
  if(scope&&typeof scope.filterNames==='function')return scope.filterNames(user,cars);
...
window.commissionAddEmployee=async function(type){if(!commissionRequireFinancialPermission())return;const st=readStore();st.employees=st.employees||{groomers:[],drivers:[],sales:[]};st.employees[type]=st.employees[type]||[];const button=document.querySelector('[data-commission-add-employee="'+String(type)+'"]'),editIndex=button&&button.hasAttribute('data-commission-edit-index')?parseInt(button.getAttribute('data-commission-edit-index'),10):-1;const name=petBlock5568_q(type+'Name')?.value?.trim();if(!name){toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('اكتب الاسم أولاً'):'اكتب الاسم أولاً');return}const carId=petBlock5568_q(type+'Car')?.value,vehicle=commissionResolveIdentity('vehicle',{vehicleId:carId},st),car=vehicle.name,from=petBlock5568_q(type+'From')?.value||currentPeriod(),to=petBlock5568_q(type+'To')?.value||'';if(!carId){toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('اختر السيارة'):'اختر السيارة');return}const current=editIndex>=0&&st.employees[type][editIndex]?st.employees[type][editIndex]:null;const overlaps=st.employees[type].some(function(e,index){if(index===editIndex||e.active===false)return false;const eid=String(e.vehicleId||commissionResolveIdentity('vehicle',{car:e.car},st).id);if(eid!==String(vehicle.id))return false;const eFrom=String(e.from||'0000-00'),eTo=String(e.to||'9999-99'),nTo=String(to||'9999-99');return eFrom<=nTo&&from<=eTo;});if(overlaps){toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('يوجد موظف مرتبط بنفس السيارة في فترة متداخلة'):'يوجد موظف مرتبط بنفس السيارة في فترة متداخلة');return}const employee=commissionResolveIdentity('employee',{employeeId:current&&current.employeeId,name:name},st),record={name:name,employeeId:employee.id,car:car,vehicleId:vehicle.id,from:from,to:to,active:current?current.active!==false:true,assignmentStatus:'assigned'};if(current)st.employees[type][editIndex]=Object.assign({},current,record);else st.employees[type].push(record);try{await writeStore(st);toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime(current?'تم تعديل الاسم':'تم حفظ الاسم'):(current?'تم تعديل الاسم':'تم حفظ الاسم'))}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch('commission employee save failed',e);toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تعذر حفظ إعدادات العمولات في Supabase'):'تعذر حفظ إعدادات العمولات في Supabase')}renderCommissionSystem()}
window.commissionEditEmployee=function(type,i){if(!commissionRequireFinancialPermission())return;const st=readStore(),row=st.employees&&st.employees[type]&&st.employees[type][i];if(!row)return;const nameInput=petBlock5568_q(type+'Name'),carInput=petBlock5568_q(type+'Car'),fromInput=petBlock5568_q(type+'From'),toInput=petBlock5568_q(type+'To'),button=document.querySelector('[data-commission-add-employee="'+String(type)+'"]');if(nameInput)nameInput.value=row.name||'';if(carInput)carInput.value=row.vehicleId||commissionResolveIdentity('vehicle',{car:row.car},st).id||'';if(fromInput)fromInput.value=row.from||currentPeriod();if(toInput)toInput.value=row.to||'';if(button){button.setAttribute('data-commission-edit-index',String(i));button.textContent='💾 '+(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('حفظ التعديل'):'حفظ التعديل')}if(nameInput){nameInput.focus();try{nameInput.scrollIntoView({behavior:'smooth',block:'center'})}catch(_e){}}}
window.commissionToggleEmployeeStatus=async function(type,i){if(!commissionRequireFinancialPermission())return;const st=readStore(),row=st.employees&&st.employees[type]&&st.employees[type][i];if(!row)return;row.active=row.active===false;try{await writeStore(st);toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تم تغيير الحالة'):'تم تغيير الحالة')}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch('commission employee status update failed',e);toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تعذر حفظ إعدادات العمولات في Supabase'):'تعذر حفظ إعدادات العمولات في Supabase')}renderCommissionSystem()}
...
function patchTab(){if(window.__PETATOE_COMMISSION_TABCHANGE_BOUND__)return;window.__PETATOE_COMMISSION_TABCHANGE_BOUND__=true;document.addEventListener('petatoe:tabchange',function(e){
...
function ensureCommissionDelegation(){
  ...
  document.addEventListener('click',function(e){
    var el=e.target&&e.target.closest?e.target.closest('[data-com-tab],[data-commission-action],[data-commission-add-employee],[data-commission-edit-employee],[data-commission-toggle-employee],[data-commission-delete-employee],[data-commission-save-settings],[data-commission-select-row]'):null;
    ...
    var edit=el.getAttribute('data-commission-edit-employee');
    if(edit){window.commissionEditEmployee&&window.commissionEditEmployee(edit,parseInt(el.getAttribute('data-commission-index')||'0',10));return;}
    var toggle=el.getAttribute('data-commission-toggle-employee');
    if(toggle){window.commissionToggleEmployeeStatus&&window.commissionToggleEmployeeStatus(toggle,parseInt(el.getAttribute('data-commission-index')||'0',10));return;}
...
function commissionsInit(){ensureCommissionDelegation();injectPanel();injectCommissionStatementPanel();patchTab();return !!(petBlock5568_q('commissions')&&petBlock5568_q('commissionStatement'));}
var commissionRuntime=window.PETATOECommissionRuntime=window.PETATOECommissionRuntime||{};
commissionRuntime.ensurePanels=commissionsInit;
commissionRuntime.renderSystem=function(){commissionsInit();return window.renderCommissionSystem&&window.renderCommissionSystem();};
commissionRuntime.renderStatement=function(){commissionsInit();return window.renderCommissionStatementPage&&window.renderCommissionStatementPage();};
if(typeof commissionRuntime.markReady==='function') commissionRuntime.markReady({ensurePanels:commissionsInit,renderSystem:commissionRuntime.renderSystem,renderStatement:commissionRuntime.renderStatement});
else {commissionRuntime.status='ready';commissionRuntime.__ready=true;}
