...
(function(){
  ...
  window.petatoeExportContractCandidatesExcel=function(){
    ...
    if(!rows.length){
      try{ toast('لا توجد بيانات للتصدير'); }catch(e){ alert('لا توجد بيانات للتصدير'); }
      return;
    }
    if(!window.XLSX){
      try{ toast('مكتبة Excel غير محملة'); }catch(e){ alert('مكتبة Excel غير محملة'); }
      return;
    }
  ...
  window.petatoeExportContractCandidatesPdf=function(){
    var rows = contractExportRows();
    if(!rows.length){
      try{ toast('لا توجد بيانات للطباعة'); }catch(e){ alert('لا توجد بيانات للطباعة'); }
      return;
    }
    var esc = function(v){
    ...
    if(!w){ try{toast('المتصفح منع نافذة الطباعة');}catch(e){alert('المتصفح منع نافذة الطباعة');} return; }
  };
