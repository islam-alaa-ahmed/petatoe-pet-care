...
(function(window, document){
  ...
  function exportExcel(){
    try{
      ...
      var rows=getRuntimeRows();
      var customers=groupCustomers(rows).filter(function(c){ return !q || lower(c.name).indexOf(q)>=0; });
      if(!window.XLSX){ if(typeof window.toast==='function') window.toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('XLSX غير متاح حاليًا'):'XLSX غير متاح حاليًا'); return; }
    ...
    }catch(e){ warn('customer360 excel export failed',e); if(typeof window.toast==='function') window.toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تعذر تصدير Customer 360'):'تعذر تصدير Customer 360'); }
  }
  function bind(){
