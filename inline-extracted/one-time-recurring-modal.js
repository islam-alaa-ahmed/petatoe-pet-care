...
(function(){
  ...
  window.exportSmartNewReturningList=function(kind){
    kind = kind==='returning'?'returning':'oneTime';
    var list=rows(kind);
    if(!list.length){notify(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('لا توجد بيانات للتصدير'):'لا توجد بيانات للتصدير');return;}
