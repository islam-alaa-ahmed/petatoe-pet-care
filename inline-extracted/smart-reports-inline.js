...
(function(){
  ...
  window.petatoeFixMonthMismatch=function(){
    if(!canWrite()){toastSafe('الصلاحية الحالية لا تسمح بالتعديل');return}
    ...
    if(c){try{_invalidateSearchIndex()}catch(e){smartReportsWarn('search index invalidation skipped after month fix',e)}; try{save()}catch(e){smartReportsWarn('save skipped after month fix',e)}; log('Month Auto Fix','Fixed '+c+' records','warn'); renderSettingsPanel(); toastSafe('تم تصحيح '+c+' سجل')}else toastSafe('لا يوجد اختلاف في الشهور')
  };
  ...
  window.petatoeBeforeSaveRecord=function(){
    if(!canWrite()){toastSafe('الصلاحية الحالية لا تسمح بالحفظ');return false}
    try{
      ...
      var v=validateRecord(r,dsRecords(),window.editingId); if(!v.ok){toastSafe('لم يتم الحفظ: '+v.errors[0]); log('Save Blocked',v.errors.join(' | '),'error');return false}
      if(v.warnings.length && !confirm('تنبيه جودة بيانات:\n- '+v.warnings.join('\n- ')+'\nهل تريد الحفظ؟')){log('Save Cancelled',v.warnings.join(' | '),'warn');return false}
