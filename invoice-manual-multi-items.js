(function(){
  ...
  async function deleteManualInvoiceFromSupabase(invoiceNo, reason){
    invoiceNo=invKey(invoiceNo);
    if(!invoiceNo) return {ok:false,error:'EMPTY_INVOICE'};
    try{
      if(window.PETATOEDataLayer&&typeof window.PETATOEDataLayer.deleteSalesInvoice==='function'){
        var res=await window.PETATOEDataLayer.deleteSalesInvoice(invoiceNo);
        if(!res||!res.ok){console.warn('[PETATOE Manual Invoice] Supabase invoice delete failed',reason||'',invoiceNo,res);return {ok:false,error:res&&res.error?res.error:res};}
        return res;
      }
      return {ok:false,error:'PETATOEDataLayer.deleteSalesInvoice is not available'};
    }catch(e){
      console.warn('[PETATOE Manual Invoice] Supabase invoice delete crashed',reason||'',invoiceNo,e);
      return {ok:false,error:e};
    }
  }
  function markRowsForFreshSupabaseInsert(rows){
    rows=Array.isArray(rows)?rows:[];
    rows.forEach(function(r){
      try{
        delete r.supabase_id;
        delete r.__petatoeManualSupabasePending;
        if(r.source==='supabase') delete r.source;
      }catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("sales/invoice-manual-multi-items.js:markRowsForFreshSupabaseInsert",e);}
    });
    return rows;
  }
  function setArr(v){
...
function renderResults(keepFocus){ensureEntryUi();var active=document.activeElement;var box=id('petEntrySearchResults');if(!box)return;var groups=filteredGroups();if(!groups.length){safeSetEntryHTML(box,'<div class="pet-entry-empty">لا توجد فواتير مطابقة من البيانات الموجودة حالياً.</div>','manual invoice empty results')}else{safeSetEntryHTML(box,groups.map(function(g){var cls=String(g.no)===String(STATE.selectedInvoice)?' active':'';var items=g.itemList.slice(0,5).join('، ')+(g.itemList.length>5?' ...':'');var safeNo=String(g.no).replace(/\\/g,'\\\\').replace(/'/g,"\\'");return '<div class="pet-entry-result-card'+cls+'"><div class="pet-entry-result-top"><b>#'+esc(g.no)+'</b><span>'+esc(g.date||'بدون تاريخ')+'</span></div><div class="pet-entry-result-meta"><div>العميل<strong>'+esc(g.client||'—')+'</strong></div><div>السيارة<strong>'+esc(g.van||'—')+'</strong></div><div>الإجمالي<strong>'+esc(mny(g.total))+'</strong></div><div>طريقة الدفع<strong>'+esc(g.pay||'—')+'</strong></div><div>عدد البنود<strong>'+esc(g.items)+'</strong></div><div>الضريبة<strong>'+esc(mny(g.tax))+'</strong></div></div><div class="pet-entry-items-line">الأصناف: '+esc(items||'—')+'</div><div class="pet-entry-result-actions"><button type="button" data-entry-action="load-invoice" data-entry-invoice-no="'+esc(g.no)+'">استعراض / تعديل</button><button type="button" class="danger" data-entry-action="delete-invoice" data-entry-invoice-no="'+esc(g.no)+'">حذف الفاتورة</button></div></div>'}).join(''),'manual invoice search results')} if(keepFocus&&active&&active.id&&id(active.id)){try{id(active.id).focus()}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("sales/invoice-manual-multi-items.js",e);}}}
  ...
  async function deleteInvoice(no){normalizeLegacyInvoices();no=invKey(no);var cnt=arr().filter(function(r){return invKey(r.invoice)===no}).length;if(!cnt){note('الفاتورة غير موجودة');return}if(!confirm(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('حذف الفاتورة رقم '):'حذف الفاتورة رقم '+no+' بالكامل؟\nعدد البنود: '+cnt))return;var delRes=await deleteManualInvoiceFromSupabase(no,'manual-invoice-delete');if(!delRes||!delRes.ok){note('فشل حذف الفاتورة من Supabase — لم يتم حذفها محلياً');return false;}setArr(arr().filter(function(r){return invKey(r.invoice)!==no}));STATE.selectedInvoice='';try{editingId=null}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("sales/invoice-manual-multi-items.js",e);}try{if(typeof _invalidateSearchIndex==='function')_invalidateSearchIndex()}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("sales/invoice-manual-multi-items.js",e);}try{save()}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("sales/invoice-manual-multi-items.js",e);}try{if(typeof renderRecords==='function')renderRecords()}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("sales/invoice-manual-multi-items.js",e);}try{if(window.PETATOESalesInvoiceReport&&window.PETATOESalesInvoiceReport.render)window.PETATOESalesInvoiceReport.render()}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("sales/invoice-manual-multi-items.js",e);}try{if(window.PETATOEDataSource&&typeof window.PETATOEDataSource.refreshSalesRecordsFromSupabase==='function')window.PETATOEDataSource.refreshSalesRecordsFromSupabase('manual-invoice-delete');}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("sales/invoice-manual-multi-items.js",e);}note('تم حذف الفاتورة');setMode('edit');return true}
  function collectItems(){try{return typeof collectEntryItems==='function'?collectEntryItems():[]}catch(e){return []}}
  async function saveEntry(){try{if(typeof window.petatoeBeforeSaveRecord==='function'&&window.petatoeBeforeSaveRecord()===false)return false}catch(e){console.warn(e)}var common={van:id('e_van')?id('e_van').value:'',date:id('e_date')?normDate(id('e_date').value):'',month:id('e_month')?(typeof normalizeMonth==='function'?normalizeMonth(id('e_month').value,id('e_date')?id('e_date').value:''):id('e_month').value):'',invoice:id('e_invoice')?invKey(id('e_invoice').value):'',client:id('e_client')?id('e_client').value:'',pay:id('e_pay')?id('e_pay').value:''};var items=collectItems();if(!common.date||!common.invoice||!common.client||!items.length){note('املأ التاريخ والعميل وصنف واحد على الأقل');return false}var old=STATE.selectedInvoice?arr().filter(function(r){return invKey(r.invoice)===STATE.selectedInvoice}):[];var rows=items.map(function(it,idx){var r=Object.assign({},old[idx]||{},it,common);r.id=(old[idx]&&old[idx].id)||(Date.now()+Math.random()+idx);r.manualEntry=true;if(!n(r.totalEx)&&n(r.price))r.totalEx=(n(r.price)*(n(r.qty)||1))-Math.abs(n(r.disc));if(!n(r.totalInc))r.totalInc=n(r.totalEx)+n(r.tax);r.disc=Math.abs(n(r.disc));return r});if(STATE.mode==='edit'&&STATE.selectedInvoice){var oldInvoice=STATE.selectedInvoice;var delRes=await deleteManualInvoiceFromSupabase(oldInvoice,'manual-invoice-edit-replace');if(!delRes||!delRes.ok){note('فشل استبدال الفاتورة في Supabase — لم يتم حفظ التعديل');return false;}rows=markRowsForFreshSupabaseInsert(rows);setArr(arr().filter(function(r){return invKey(r.invoice)!==oldInvoice}).concat(rows));STATE.selectedInvoice=common.invoice}else{setArr(arr().concat(rows));try{if(typeof setNextManualInvoiceAfter==='function')setNextManualInvoiceAfter(common.invoice)}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("sales/invoice-manual-multi-items.js",e);}}try{if(typeof _invalidateSearchIndex==='function')_invalidateSearchIndex()}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("sales/invoice-manual-multi-items.js",e);}try{save()}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("sales/invoice-manual-multi-items.js",e);}try{updateItemDatalist()}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("sales/invoice-manual-multi-items.js",e);}try{if(typeof renderRecords==='function')renderRecords()}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("sales/invoice-manual-multi-items.js",e);}try{if(window.PETATOESalesInvoiceReport&&window.PETATOESalesInvoiceReport.render)window.PETATOESalesInvoiceReport.render()}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("sales/invoice-manual-multi-items.js",e);}try{document.dispatchEvent(new CustomEvent('petatoe:record-saved',{detail:{mode:STATE.mode,invoice:common.invoice}}))}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("sales/invoice-manual-multi-items.js",e);}note((STATE.mode==='edit'?'تم تعديل الفاتورة رقم ':'تم حفظ الفاتورة رقم ')+common.invoice);if(STATE.mode==='edit')setTimeout(function(){loadInvoice(common.invoice)},50);else setMode('new');return true}
