(function(){
  ...
  var SCREEN_ALIASES = {
    ...
    smart:'reports', customer360:'customers', executive:'executive',
    treasury:'treasury', warehouses:'warehouses', obligations:'obligations', expenses:'expenses',
  ...
  function canOpen(screen){
    ...
    if(screen==='dashboardManagement' || screen==='dashboardOperations') return true;
  ...
  function hasAnyAction(u, screen){
    ...
    try{
      if(window.PETATOEPermissions && typeof window.PETATOEPermissions.can==='function'){
        return ['view','add','edit','delete','export','approve'].some(function(a){return !!window.PETATOEPermissions.can(u.id, screen, a)});
      }
    }catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch('navigation/navigation-permissions.js',e);}
  ...
  function apply(root){
    ...
    Array.prototype.forEach.call(root.querySelectorAll('button[data-tab],button[data-settings-main]'),function(btn){
      ...
      var allowed = screen==='dashboardManagement' ? (hasAnyAction(u,'dashboardManagement') || hasAnyAction(u,'dashboardOperations')) : hasAnyAction(u, screen);
      btn.style.display = allowed ? '' : 'none';
      btn.classList.toggle('pet-nav-hidden-by-permission', !allowed);
    ...
    Array.prototype.forEach.call(root.querySelectorAll('.pet-v142-group'),function(g){
      ...
      g.classList.toggle('pet-nav-group-hidden-by-permission', !visible);
    });
  }
