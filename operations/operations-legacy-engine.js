...
(function(){
  ...
  function canonicalText(path,key,params){
    ...
    if((value==null||value==='')&&path.indexOf('operations.reports.')===0)value=read('enterpriseUiCertification.operationsReports.'+path.slice('operations.reports.'.length));
  ...
  var pendingMasterCustomersImport=null;
  var masterCustomersImportBusy=false;
  ...
  function mergeMasterCustomerInto(master,c,options){
    ...
    master=normalizeMasterData(master||cloneDefaultMaster());
    var now=new Date().toISOString(), found=-1;
    ...
    return {changed:before!==after,created:found===-1,updated:found>-1,master:master,customer:merged};
  }
  function upsertMasterCustomer(c,options){
    var master=readMasterData(), result=mergeMasterCustomerInto(master,c,options);
    if(!result||!result.customer)return false;
    writeMasterData(result.master); return true;
  }
  ...
  function customerImportProgress(percent,statusKey,params){
    ...
    var bar=byId('appointmentMasterCustomersImportProgressBar');
    var value=byId('appointmentMasterCustomersImportProgressValue');
    var status=byId('appointmentMasterCustomersImportStatus');
    percent=Math.max(0,Math.min(100,Math.round(Number(percent)||0)));
    if(panel)panel.hidden=false;
    if(bar){bar.style.width=percent+'%';bar.setAttribute('aria-valuenow',String(percent));}
    if(value)value.textContent=percent+'%';
    if(status&&statusKey)status.textContent=opCustomerT(statusKey,params,statusKey);
  }
  function setCustomerImportActions(reviewReady,busy){
    var approve=byId('appointmentMasterCustomersImportApprove');
    var cancel=byId('appointmentMasterCustomersImportCancel');
    if(approve){approve.hidden=!reviewReady;approve.disabled=!!busy;}
    if(cancel){cancel.hidden=!reviewReady;cancel.disabled=!!busy;}
  }
  function clearMasterCustomersImportReview(){
    pendingMasterCustomersImport=null;masterCustomersImportBusy=false;
    var panel=byId('appointmentMasterCustomersImportReview');if(panel)panel.hidden=true;
  ...
  function isRealXlsxReady(){
    return !!(window.XLSX && !window.__PETATOE_XLSX_STUB__ && typeof window.XLSX.read==='function' && window.XLSX.utils && typeof window.XLSX.utils.sheet_to_json==='function');
  }
  function ensureCustomerImportXlsxReady(){
    if(isRealXlsxReady())return Promise.resolve(true);
    var gate=window.PETATOEMobileStartupGate;
    if(!gate||typeof gate.ensureGroup!=='function')return Promise.reject(new Error('PETATOE_XLSX_LOADER_UNAVAILABLE'));
    customerImportProgress(1,'import.loadingExcelLibrary');
    return Promise.resolve(gate.ensureGroup('xlsx')).then(function(){
  ...
  function handleMasterCustomersExcelImport(input){
    ...
    if(!file||masterCustomersImportBusy)return;
    ...
    var readSelectedFile=function(){
      var reader=new FileReader();
      reader.onprogress=function(e){if(e&&e.lengthComputable)customerImportProgress(Math.max(2,Math.min(55,(e.loaded/e.total)*55)),'import.readingFile')};
      reader.onload=function(e){
        try{
          customerImportProgress(58,'import.parsingFile');
      ...
      reader.onerror=function(){masterCustomersImportBusy=false;customerImportProgress(0,'import.readFailed');setCustomerImportActions(false,false);if(input)input.value='';};
      reader.readAsArrayBuffer(file);
    };
    var ext=String(file.name||'').toLowerCase();
    var readiness=ext.endsWith('.csv')?Promise.resolve(true):ensureCustomerImportXlsxReady();
    readiness.then(readSelectedFile).catch(function(err){
      console.error(err);masterCustomersImportBusy=false;customerImportProgress(0,'import.excelUnavailable');setCustomerImportActions(false,false);
  ...
  function approveMasterCustomersExcelImport(){
    ...
    if(!storage||typeof storage.writeMasterDataConfirmed!=='function'){
      customerImportProgress(0,'import.supabaseUnavailable');return;
    }
  ...
  function cancelMasterCustomersExcelImport(){clearMasterCustomersImportReview();}
  ...
  function normalizePhone(v){return String(v||'').replace(/[^0-9+]/g,'').trim()}
  function customerSearchText(v){return String(v||'').trim().toLowerCase()}
  function hideCustomerSearchResults(){var box=byId('appointmentCustomerSearchResults');if(!box)return;box.hidden=true;safeHtml(box,'','operations customer smart search clear')}
  function setNewCustomerMode(enabled){var fields=byId('appointmentNewCustomerFields');if(fields)fields.hidden=!enabled;var search=byId('appointmentCustomerSearch');if(search)search.setAttribute('aria-expanded','false');if(enabled){setVal('appointmentCustomerId','');hideCustomerSearchResults();}}
  function appointmentCustomerSearchLabel(profile){return [profile&&profile.client,profile&&profile.phone].filter(Boolean).join(' — ')}
  function rankCustomerProfile(profile,query){
    var q=customerSearchText(query), qPhone=normalizePhone(query), name=customerSearchText(profile&&profile.client), phone=normalizePhone(profile&&profile.phone);
    if(!q)return 99;
    if(qPhone&&phone===qPhone)return 0;
    if(qPhone&&phone.indexOf(qPhone)===0)return 1;
    if(name===q)return 2;
    if(name.indexOf(q)===0)return 3;
    if(name.indexOf(q)>-1)return 4;
    if(qPhone&&phone.indexOf(qPhone)>-1)return 5;
    return 99;
  }
  function searchAppointmentCustomers(query){
    var input=byId('appointmentCustomerSearch'), box=byId('appointmentCustomerSearchResults');
    query=String(query==null?(input&&input.value||''):query).trim();
    var selectedId=String(val('appointmentCustomerId')||'').trim();
    if(selectedId){
      var selected=buildCustomerProfiles().find(function(c){return String(c.key||'')===selectedId});
      if(!selected||query!==appointmentCustomerSearchLabel(selected)){
        ['appointmentCustomerId','appointmentClient','appointmentPhone','appointmentAddress','appointmentGoogleMapUrl'].forEach(function(id){setVal(id,'')});
      }
    }
    if(!box||!query){hideCustomerSearchResults();return [];}
    var rows=buildCustomerProfiles().map(function(profile){return {profile:profile,rank:rankCustomerProfile(profile,query)}}).filter(function(x){return x.rank<99}).sort(function(a,b){return a.rank-b.rank||String(b.profile.lastVisit||'').localeCompare(String(a.profile.lastVisit||''))||String(a.profile.client||'').localeCompare(String(b.profile.client||''));}).slice(0,8);
    var html=rows.length?rows.map(function(x){var c=x.profile;return '<button type="button" class="appointment-customer-search-result" data-op-click="selectAppointmentCustomer" data-op-arg1="'+esc(c.key)+'"><b>'+esc(c.client||opAppointmentT('customerFallback',null,'عميل'))+'</b><span>'+esc(c.phone||'-')+'</span>'+(c.address?'<small>'+esc(c.address)+'</small>':'')+'</button>'}).join(''):'<div class="appointment-customer-search-empty">'+esc(opAppointmentT('customerSearchNoResults',null,'لا يوجد عميل مطابق'))+'</div>';
    safeHtml(box,html,'operations customer smart search results');box.hidden=false;if(input)input.setAttribute('aria-expanded','true');return rows.map(function(x){return x.profile});
  }
  function selectAppointmentCustomer(key){
    var found=buildCustomerProfiles().find(function(c){return String(c.key||'')===String(key||'')});
    if(!found)return;
    setVal('appointmentCustomerId',found.key);setVal('appointmentClient',found.client||'');setVal('appointmentPhone',found.phone||'');setVal('appointmentCustomerSearch',appointmentCustomerSearchLabel(found));
    setNewCustomerMode(false);hideCustomerSearchResults();applyCustomerSuggestion(byId('appointmentCustomerSearch'));
  }
  function syncNewCustomerFields(){
    var name=String(val('appointmentClient')||'').trim(),phone=String(val('appointmentPhone')||'').trim();
    setVal('appointmentCustomerSearch',[name,phone].filter(Boolean).join(' — '));
  }
  ...
  function renderClientSuggestions(){
    var oldClients=byId('appointmentClientSuggestions'), oldPhones=byId('appointmentPhoneSuggestions');
    if(oldClients)safeHtml(oldClients,'','operations legacy customer name suggestions cleared');
    if(oldPhones)safeHtml(oldPhones,'','operations legacy customer phone suggestions cleared');
  }
  ...
  function newCustomer(){
    ['appointmentCustomerId','appointmentCustomerSearch','appointmentClient','appointmentPhone','appointmentAnimalType','appointmentBreed','appointmentSize','appointmentPetName','appointmentAddress','appointmentGoogleMapUrl'].forEach(function(id){setVal(id,'')});
  ...
  function applyCustomerSuggestion(source){
    ...
    setVal('appointmentCustomerSearch',appointmentCustomerSearchLabel(found));
  ...
  function reportTable(title,icon,rows,totalLabel,limit){
    ...
    var body=shown.length?shown.map(function(r,i){return '<tr><td>'+(i+1)+'</td><td>'+esc(r.name)+'</td><td>'+r.count+'</td><td>'+r.percent+'%</td></tr>'}).join(''):'<tr><td colspan="4" class="appointments-empty">'+esc(opReportT('noReportData'))+'</td></tr>';
  ...
  function localReportTable(key,title,icon,baseRows,field,fallback,totalLabel,limit,transform){
    ...
    var body=shown.length?shown.map(function(r,i){return '<tr><td>'+(i+1)+'</td><td>'+esc(r.name)+'</td><td>'+Number(r.count||0)+'</td><td>'+pct(Number(r.count||0),total)+'%</td></tr>'}).join(''):'<tr><td colspan="4" class="appointments-empty">'+esc(opReportT('noReportData'))+'</td></tr>';
    if(hidden){body+='<tr class="appointments-report-more-row"><td>+</td><td>'+esc(opReportT('otherHiddenItems'))+'</td><td>'+hidden+'</td><td>'+pct(hidden,total)+'%</td></tr>';}
    var controls=appointmentReportFiltersHtml(key,baseRows,shown.length,rows.length);
    var more=rows.length>shown.length?'<div class="appointments-finance-load-more"><span class="appointments-finance-more-count">'+esc(opReportT('showingItems',{shown:shown.length,total:rows.length}))+'</span><button type="button" data-op-click="showMoreAppointmentLocalReportRows" data-op-arg1="'+esc(key)+'" onclick="(window.PETATOEOperations||window.PETATOEAppointments).showMoreAppointmentLocalReportRows(\''+esc(key)+'\');return false;">'+esc(opReportT('showMore'))+'</button></div>':'<div class="appointments-finance-load-more"><span>'+esc(opReportT('showingItems',{shown:shown.length,total:rows.length}))+'</span></div>';
    return '<div class="appointments-report-table-card appointments-finance-report-local"><div class="appointments-report-title"><span class="appointments-report-icon">'+icon+'</span><div><h4>'+esc(title)+'</h4><p>'+esc(opReportT('matchingRecords'))+': '+filtered.length+' | '+esc(opReportT('totalItems'))+': '+total+'</p></div></div>'+controls+'<div class="appointments-report-table-wrap"><table class="appointments-report-table"><thead><tr><th>#</th><th>'+esc(opReportT('item'))+'</th><th>'+esc(opReportT('appointmentCount'))+'</th><th>'+esc(opReportT('percentage'))+'</th></tr></thead><tbody>'+body+'</tbody><tfoot><tr><td colspan="2">'+esc(totalLabel||opT('total'))+'</td><td>'+total+'</td><td>100%</td></tr></tfoot></table></div>'+more+'</div>';
  }
  ...
  function renderReports(){
    ...
    safeHtml(el,cards+'<div class="appointments-report-tables appointments-appointment-reports-fullwidth">'
      ...
      +localReportTable('by_status',opReportT('byStatusTitle'),'📌',rows,function(r){return normalizeStatus(r.status)},opReportT('scheduled'),opReportT('totalStatuses'))
      +localReportTable('by_groomer',opReportT('byGroomerTitle'),'✂️',rows,'groomer',opReportT('noGroomer'),opReportT('totalGroomers'))
      +localReportTable('by_driver',opReportT('byDriverTitle'),'🚗',rows,'driver',opReportT('noDriver'),opReportT('totalDrivers'))
      +localReportTable('by_vehicle',opReportT('byVehicleTitle'),'🚐',rows,'vehicle',opReportT('noVehicle'),opReportT('totalVehicles'))
      +localReportTable('by_payment',opReportT('byPaymentTitle'),'💳',rows,'paymentMethod',opReportT('unknown'),opReportT('totalPayments'))
      +localReportTable('by_collection',opReportT('byCollectionTitle'),'🧾',rows,function(r){var item=dataset&&dataset.normalizeRow?dataset.normalizeRow(r,datasetOptions):null;return item?(item.paymentClass==='paid'?'محصل بالكامل':item.paymentClass==='partial'?'محصل جزئي':'غير محصل'):(r.collectionStatus||'غير محدد')},opReportT('unknown'),opReportT('totalCollectionStatuses'))
      +localReportTable('repeat_clients',opReportT('repeatCustomersTitle'),'👥',rows,null,opReportT('unknownCustomer'),opReportT('totalRepeatAppointments'),0,function(filteredRows){
  ...
  function setCalendarView(view){calendarView=view==='month'?'month':'timeline';document.querySelectorAll('[data-calendar-view]').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-calendar-view')===calendarView)});renderCalendar()}
  function shiftCalendarMonth(delta){var d=calendarBaseDate();d=new Date(d.getFullYear(),d.getMonth()+Number(delta||0),1);setVal('appointmentCalendarDate',dateKey(d));calendarView='month';renderCalendar()}
  function setCalendarToday(){setVal('appointmentCalendarDate',today());renderCalendar()}
  function calendarRange(){
    var d=calendarBaseDate();
    if(calendarView==='month')return {from:monthStart(d),to:monthEnd(d),title:opT('calendar.monthly')};
    var k=dateKey(d);return {from:k,to:k,title:opT('calendar.timeline')};
  }
  ...
  function closeCalendarDayDetails(){var modal=byId('appointmentsCalendarDayModal');if(modal&&modal.parentNode)modal.parentNode.removeChild(modal)}
  ...
  function openCalendarDayDetails(day){
    ...
    var columns=groups.map(function(g){return '<section class="appointments-calendar-vehicle-column"><header><div><span class="appointments-calendar-vehicle-dot"></span><h4>'+esc(g.name)+'</h4></div><b>'+g.rows.length+' '+esc(opT(g.rows.length===1?'calendar.appointment':'calendar.appointments'))+'</b></header><div class="appointments-calendar-vehicle-list">'+g.rows.map(calendarAppointmentCard).join('')+'</div></section>'}).join('');
    safeHtml(modal,'<div class="appointments-calendar-day-backdrop" data-op-click="closeCalendarDayDetails"></div><div class="appointments-calendar-day-panel" role="dialog" aria-modal="true"><header class="appointments-calendar-day-head"><div><h3>'+esc(opT('calendar.detailsTitle'))+' — '+esc(calendarDayTitle(day))+'</h3><p>'+esc(opT('calendar.totalAppointments',{count:rows.length}))+'</p></div><button type="button" class="appointments-calendar-day-close" data-op-click="closeCalendarDayDetails">×</button></header><div class="appointments-calendar-vehicle-columns">'+columns+'</div><footer><button type="button" class="btn btn-ghost" data-op-click="closeCalendarDayDetails">'+esc(opT('calendar.close'))+'</button></footer></div>','operations calendar vehicle details modal');
    document.body.appendChild(modal);
  }
  function renderCalendarTimeline(el,rows,range){
    if(!rows.length){safeHtml(el,'<div class="appointments-empty appointments-calendar-empty">'+esc(opT('calendar.noAppointments'))+'</div>','operations calendar timeline empty');return}
    var groups={};rows.forEach(function(r){var d=String(r.date||'');(groups[d]=groups[d]||[]).push(r)});
    safeHtml(el,Object.keys(groups).sort().map(function(day){return '<div class="appointments-day-card"><div class="appointments-day-head"><b>'+esc(calendarDayTitle(day))+'</b><span>'+groups[day].length+' '+esc(opT('calendar.appointments'))+'</span></div><div class="appointments-timeline">'+groups[day].map(function(r){var conflictsHere=findConflicts(r,rows).length>0;return '<div class="appointments-timeline-item '+(conflictsHere?'has-conflict':'')+'"><div class="appointments-time"><b>'+esc(r.start||'--:--')+'</b><small>'+esc(r.end||'')+'</small></div><div class="appointments-event"><b>'+esc(r.client||opT('fallback.unknownCustomer'))+'</b><p>'+esc([r.service,r.animalType,r.breed,r.size,r.petName].filter(Boolean).join(' - '))+'</p><div class="appointments-event-meta"><span>✂️ '+esc(r.groomer||'-')+'</span><span>🚗 '+esc(r.driver||'-')+'</span><span>🚐 '+esc(r.vehicle||'-')+'</span><span class="appointments-status '+statusClass(r.status)+'">'+esc(normalizeStatus(r.status))+'</span></div><small>'+esc(r.address||'')+'</small></div></div>'}).join('')+'</div></div>'}).join(''),'operations calendar timeline');
  }
  function renderCalendarMonth(el,rows){
    var base=calendarBaseDate(), year=base.getFullYear(), month=base.getMonth(), first=new Date(year,month,1), start=new Date(year,month,1-first.getDay()), byDay={};
    activeCalendarRows(rows).forEach(function(r){var d=String(r.date||'');(byDay[d]=byDay[d]||[]).push(r)});
    var weekdays=[], locale=calendarLocale();for(var w=0;w<7;w++){try{weekdays.push(new Intl.DateTimeFormat(locale,{weekday:'short'}).format(new Date(2026,7,2+w)))}catch(_){weekdays.push(String(w+1))}}
    var cells=[];for(var i=0;i<42;i++){var d=new Date(start.getFullYear(),start.getMonth(),start.getDate()+i), key=dateKey(d), dayRows=byDay[key]||[], vehicleGroups=calendarVehicleGroups(dayRows), inMonth=d.getMonth()===month, isToday=key===today();
      var vehicleSummary=vehicleGroups.map(function(g){return '<div class="appointments-month-vehicle"><span>'+esc(g.name)+'</span><b>'+g.rows.length+'</b></div>'}).join('');
      cells.push('<article class="appointments-month-day '+(inMonth?'':'outside-month')+' '+(isToday?'is-today':'')+'"><div class="appointments-month-day-head"><b>'+d.getDate()+'</b><span>'+dayRows.length+' '+esc(opT('calendar.appointments'))+'</span></div>'+(dayRows.length?'<div class="appointments-month-vehicles">'+vehicleSummary+'</div><button type="button" class="appointments-month-details" data-op-click="openCalendarDayDetails" data-op-arg1="'+key+'">'+esc(opT('calendar.viewDetails'))+'</button>':'<div class="appointments-month-empty">'+esc(opT('calendar.noAppointments'))+'</div>')+'</article>');
    }
    safeHtml(el,'<div class="appointments-month-shell"><div class="appointments-month-title">'+esc(calendarMonthTitle(base))+'</div><div class="appointments-month-weekdays">'+weekdays.map(function(x){return '<span>'+esc(x)+'</span>'}).join('')+'</div><div class="appointments-month-grid">'+cells.join('')+'</div></div>','operations monthly vehicle calendar');
  }
  ...
  window.PETATOEOperationsAppointmentsInternal={
    ...
    searchAppointmentCustomers:searchAppointmentCustomers,
    selectAppointmentCustomer:selectAppointmentCustomer,
    syncNewCustomerFields:syncNewCustomerFields,
