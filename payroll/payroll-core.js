...
(function(){
  ...
  window.PETATOEPayroll={
    ...
    saveEmployeeCodeConfig:function(){var cfg={prefix:(byId('payEmpCodePrefix')||{}).value,next:(byId('payEmpCodeNext')||{}).value,digits:(byId('payEmpCodeDigits')||{}).value};saveEmployeeConfig(cfg);toastMsg(payrollT('config.employee.saved','تم حفظ تهيئة أكواد الموظفين'));render()},
    saveJobType:function(){var inp=byId('payJobTypeName');var old=(byId('payJobTypeOriginal')||{}).value||'';var val=inp?String(inp.value||'').trim():'';if(!val){toastMsg(payrollT('config.jobs.enterName','اكتب اسم الوظيفة'));return}var arr=jobTypes();if(arr.some(function(j){return norm(j)===norm(val)&&norm(j)!==norm(old)})){toastMsg(payrollT('config.jobs.duplicate','الوظيفة موجودة بالفعل'));return}if(old){arr=arr.map(function(j){return norm(j)===norm(old)?val:j});var emps=employees().map(function(e){if(norm(e.job)===norm(old))e.job=val;return e});saveEmployees(emps);toastMsg(payrollT('config.jobs.updatedEmployees','تم تعديل الوظيفة وتحديث الموظفين المرتبطين بها'))}else{arr.push(val);toastMsg(payrollT('config.jobs.added','تمت إضافة الوظيفة'))}saveJobTypes(arr);render()},
    ...
    saveSlip:function(){var form=readSlipForm('draft');if(!validateSlipBeforePersist(form))return;var existing=slips().find(function(x){return String(x.id)===String(form.id)||sameSlipMonth(x,form)});if(existing&&String(existing.status||'draft')!=='draft'){toastMsg(payrollT('workflow.cannotSaveApprovedAsDraft','لا يمكن حفظ كشف دخل دورة الاعتماد كمسودة. استخدم إلغاء الاعتماد أولاً.'));return}var s=upsertSlip(form);if(!s)return;notifyAfterPersist(s._persistPromise,payrollT('workflow.draftSaved','تم حفظ كشف الراتب كمسودة'));delete s._persistPromise;state.editSlipId=s.id;render()},
    sendToBoard:function(){var form=readSlipForm('pending_board');if(!validateSlipBeforePersist(form))return;var existing=slips().find(function(x){return String(x.id)===String(form.id)||sameSlipMonth(x,form)});if(existing&&String(existing.status||'draft')!=='draft'){toastMsg(payrollT('workflow.sendOnlyFromDraft','لا يمكن إرسال الكشف للاعتماد إلا من حالة مسودة'));return}var s=upsertSlip(form);if(!s)return;notifyAfterPersist(s._persistPromise,payrollT('workflow.sentToBoard','تم إرسال الكشف للاعتماد المبدئي'));delete s._persistPromise;state.editSlipId=s.id;render()},
    ...
    deleteSlip:function(id){var s=slips().find(function(x){return String(x.id)===String(id)});if(!s){toastMsg(payrollT('messages.slipNotFound','لم يتم العثور على كشف الراتب'));return}if(String(s.status||'draft')!=='draft'){toastMsg(payrollT('workflow.cannotDeleteApprovedDetailed','لا يمكن حذف كشف دخل دورة الاعتماد. استخدم إلغاء الاعتماد أولاً ثم احذف المسودة.'));return}if(!confirm(payrollT('confirm.deleteSlipPermanent','حذف كشف الراتب نهائيًا من الرواتب الشهرية؟')))return;setSlipsCache(slips().filter(function(x){return !sameSlipMonth(x,s)&&String(x.id)!==String(s.id)}));notifyAfterPersist(removeOneSlip(s),payrollT('workflow.slipDeleted','تم حذف كشف الراتب'));if(state.editSlipId===id)state.editSlipId='';render();renderSalarySlip();},
    ...
    cancelApproval:function(id){
      ...
      s.updatedAt=at;
      setSlipsCache(arr);
      notifyAfterPersist(persistOneSlip(s),payrollT('workflow.cancelledOneStep','تم إلغاء الاعتماد خطوة واحدة للخلف'));
      render();
      renderSalarySlip();
    },
