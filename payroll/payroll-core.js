...
(function(){
  ...
  window.PETATOEPayroll={
    ...
    saveJobType:function(){var inp=byId('payJobTypeName');var old=(byId('payJobTypeOriginal')||{}).value||'';var val=inp?String(inp.value||'').trim():'';if(!val){toastMsg(payrollT('config.jobs.enterName','اكتب اسم الوظيفة'));return}var arr=jobTypes();if(arr.some(function(j){return norm(j)===norm(val)&&norm(j)!==norm(old)})){toastMsg('الوظيفة موجودة بالفعل');return}if(old){arr=arr.map(function(j){return norm(j)===norm(old)?val:j});var emps=employees().map(function(e){if(norm(e.job)===norm(old))e.job=val;return e});saveEmployees(emps);toastMsg('تم تعديل الوظيفة وتحديث الموظفين المرتبطين بها')}else{arr.push(val);toastMsg('تمت إضافة الوظيفة')}saveJobTypes(arr);render()},
    ...
    deleteSlip:function(id){var s=slips().find(function(x){return String(x.id)===String(id)});if(!s){toastMsg(payrollT('messages.slipNotFound','لم يتم العثور على كشف الراتب'));return}if(String(s.status||'draft')!=='draft'){toastMsg('لا يمكن حذف كشف دخل دورة الاعتماد. استخدم إلغاء الاعتماد أولاً ثم احذف المسودة.');return}if(!confirm(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('حذف كشف الراتب نهائيًا من الرواتب الشهرية؟'):'حذف كشف الراتب نهائيًا من الرواتب الشهرية؟'))return;setSlipsCache(slips().filter(function(x){return !sameSlipMonth(x,s)&&String(x.id)!==String(s.id)}));notifyAfterPersist(removeOneSlip(s),'تم حذف كشف الراتب');if(state.editSlipId===id)state.editSlipId='';render();renderSalarySlip();},
    ...
    employeeApprove:function(id){var s=slips().find(function(x){return x.id===id});if(!s||!canEmployeeSee(s)){toastMsg(payrollT('workflow.cannotApproveOthers','لا يمكنك اعتماد كشف غير خاص بك'));return}if(String(s.status||'draft')!=='board_approved'){toastMsg('لا يمكن موافقة الموظف إلا بعد اعتماد رئيس مجلس الإدارة');return}notifyAfterPersist(setStatus(id,'employee_approved',{employeeApprovedAt:new Date().toISOString(),employeeApprovedBy:(currentUser().fullName||currentUser().username),employeeNote:''}),'تمت موافقة الموظف')},
    employeeObject:function(id){var s=slips().find(function(x){return x.id===id});if(!s||!canEmployeeSee(s)){toastMsg(payrollT('workflow.cannotObjectOthers','لا يمكنك الاعتراض على كشف غير خاص بك'));return}if(String(s.status||'draft')!=='board_approved'){toastMsg('لا يمكن الاعتراض إلا على كشف معتمد مبدئيًا من رئيس مجلس الإدارة');return}var note=(byId('empNote_'+id)||{}).value||'';if(!note.trim()){toastMsg('اكتب سبب الاعتراض');return}notifyAfterPersist(setStatus(id,'employee_objection',{employeeObjectedAt:new Date().toISOString(),employeeNote:note.trim()}),'تم تسجيل الاعتراض')},
