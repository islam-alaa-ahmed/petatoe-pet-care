...
(function(){
  ...
  async function setStatus(id,st,extra){var before=slips();var arr=cloneVal(before);var slip=arr.find(function(x){return x.id===id});if(!slip)return false;slip.status=st;Object.assign(slip,extra||{});slip.updatedAt=new Date().toISOString();setSlipsCache(arr);try{await persistOneSlip(slip);render();renderSalarySlip();return true}catch(e){setSlipsCache(before);render();renderSalarySlip();throw e}}
  ...
  window.PETATOEPayroll={
    ...
    deleteEmployee:async function(id){if(!requirePayrollManage())return;if(!confirm(payrollT('confirm.deleteEmployee','حذف الموظف من قسم الرواتب؟')))return;try{await saveEmployees(employees().filter(function(e){return e.id!==id}));render()}catch(_e){}},
    ...
    accountsApprove:function(id){if(!isAccounts()){toastMsg(payrollT('workflow.accountsPermissionRequired','هذه الصلاحية للحسابات'));return}var s=slips().find(function(x){return x.id===id});if(!s||s.status!=='employee_approved'){toastMsg(payrollT('workflow.employeeApprovalRequired','لا يمكن اعتماد الصرف قبل موافقة الموظف'));return}notifyAfterPersist(setStatus(id,'accounts_approved',{accountsApprovedAt:new Date().toISOString(),accountsApprovedBy:(currentUser().fullName||currentUser().username)}),payrollT('workflow.accountsApproved','تم اعتماد الصرف'))},
    markPaid:function(id){if(!isAccounts()){toastMsg(payrollT('workflow.accountsPermissionRequired','هذه الصلاحية للحسابات'));return}var s=slips().find(function(x){return x.id===id});if(!s||s.status!=='accounts_approved'){toastMsg(payrollT('workflow.accountsApprovalRequired','لا يمكن تسجيل الصرف قبل اعتماد الحسابات'));return}notifyAfterPersist(setStatus(id,'paid',{paidAt:new Date().toISOString()}),payrollT('workflow.paidRecorded','تم تسجيل الصرف'))},
