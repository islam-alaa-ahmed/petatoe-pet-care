...
(function(){
  ...
  function monthlyReportFiltersHtml(){ensureReportDefaults();var years=reportYears();var year=state.reportYear||currentArchiveYear();var months=reportMonths(year);var yOpts=years.map(function(y){return '<option value="'+esc(y)+'" '+(String(y)===String(year)?'selected':'')+'>'+esc(y)+'</option>'}).join('');var mOpts='<option value="">كل الشهور</option>'+months.map(function(m){return '<option value="'+esc(m)+'" '+(String(m)===String(state.reportMonth||'')?'selected':'')+'>'+esc(monthName(m))+'</option>'}).join('');return '<div class="payroll-grid-4 payroll-archive-filters"><div class="payroll-field"><label>السنة</label><select id="payMonthlyReportYear" data-payroll-change="monthly-report-year">'+yOpts+'</select></div><div class="payroll-field"><label>'+esc(payrollT('ui.month','الشهر'))+'</label><select id="payMonthlyReportMonth" data-payroll-change="monthly-report-month">'+mOpts+'</select></div><div class="payroll-field"><label>طريقة الصرف</label><select id="payMonthlyReportPayment" data-payroll-change="monthly-report-payment">'+paymentOptions(state.reportPayment||'',true)+'</select></div><div class="payroll-field payroll-filter-actions"><label>&nbsp;</label><button class="btn btn-primary" data-payroll-action="export-monthly-report-pdf">PDF 🖨️</button><button class="btn btn-ghost" data-payroll-action="export-monthly-report-csv">Excel 📥</button></div></div>'}
