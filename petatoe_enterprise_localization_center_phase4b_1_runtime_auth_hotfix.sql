-- PETATOE v9 ELC Phase 4B.1
-- Fixes custom-session authorization for Localization Center RPCs.
...
begin
  if nullif(btrim(coalesce(p_actor_user_id,'')),'') is null
     or nullif(btrim(coalesce(p_session_token,'')),'') is null then
    return 'viewer';
  end if;

  begin
    v_user_id := p_actor_user_id::uuid;
  ...
  select lower(coalesce(au.role_code, au.legacy_payload->>'role', 'viewer'))
    into v_role
