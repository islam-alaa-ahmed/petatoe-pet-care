(function(){
  ...
  function petatoeShowContractCandidateReasonLegacy(idx){
    ...
    if(!d){
      alert('لم يتم العثور على تفاصيل سبب الترشيح لهذا العميل. افتح التقرير مرة أخرى ثم جرّب.');
      return;
    }
  ...
  window.petatoeExportContractCandidatesExcel=function(){
    try{
      ...
      if(!rows.length){alert('لا توجد بيانات لتصدير تقرير العملاء المرشحين للعقود.');return;}
    ...
    }catch(err){console.error(err);alert('تعذر تصدير تقرير العملاء المرشحين Excel.');}
  };
  window.petatoeExportContractCandidatesPdf=function(){
    try{
      var table=document.getElementById('contractCandidatesTable');
      if(!table){alert('تقرير العملاء المرشحين للعقود غير مفتوح حالياً.');return;}
      var rows=contractRows();
      ...
      if(!w){alert('المتصفح منع نافذة الطباعة. اسمح بالنوافذ المنبثقة ثم جرّب مرة أخرى.');return;}
    ...
    }catch(err){console.error(err);alert('تعذر تصدير تقرير العملاء المرشحين PDF.');}
  };
})();
