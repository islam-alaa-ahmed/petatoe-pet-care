...
const release=fs.readFileSync(path.join(root,'RELEASE_VERSION.txt'),'utf8');if(!release.includes('PETATOE v9.4.14')||!release.includes('PETATOE_V9_4_14_SMART_REPORTS_DATA_READINESS_RECOVERY'))failures.push('Release metadata is not synchronized to v9.4.14.');
const runtime=fs.readFileSync(path.join(root,'i18n/localization-center/runtime.js'),'utf8');if(!runtime.includes("VERSION='9.4.14-smart-reports-performance'"))failures.push('Localization runtime version is not synchronized to v9.4.14.');
