...
const release=fs.readFileSync(path.join(root,'RELEASE_VERSION.txt'),'utf8');if(!release.includes('PETATOE v9.4.17')||!release.includes('PETATOE_V9_4_17_DISPLAY_LAYER_LOCALIZATION_PERFORMANCE'))failures.push('Release metadata is not synchronized to v9.4.17.');
const runtime=fs.readFileSync(path.join(root,'i18n/localization-center/runtime.js'),'utf8');if(!runtime.includes("VERSION='9.4.17-display-layer-localization-performance'"))failures.push('Localization runtime version is not synchronized to v9.4.17.');
