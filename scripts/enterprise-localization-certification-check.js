...
const release=fs.readFileSync(path.join(root,'RELEASE_VERSION.txt'),'utf8');if(!release.includes('PETATOE v9.4.3')||!release.includes('ELC_V9_4_3_ENTERPRISE_LOCALIZATION_CERTIFICATION'))failures.push('Release metadata is not synchronized to v9.4.3.');
const runtime=fs.readFileSync(path.join(root,'i18n/localization-center/runtime.js'),'utf8');if(!runtime.includes("VERSION='9.4.3-single-source-enforcement'")&&!runtime.includes("VERSION='9.4.3-enterprise-certification'"))failures.push('Localization runtime version is not synchronized to v9.4.3.');
