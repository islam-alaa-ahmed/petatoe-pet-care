...
const release=fs.readFileSync(path.join(root,'RELEASE_VERSION.txt'),'utf8');if(!release.includes('PETATOE v9.4.20')||!release.includes('PETATOE_V9_4_20_SMART_REPORTS_TRANSLATION_STABILITY_PERFORMANCE_CERTIFICATION'))failures.push('Release metadata is not synchronized to v9.4.19.');
const runtime=fs.readFileSync(path.join(root,'i18n/localization-center/runtime.js'),'utf8');if(!runtime.includes("VERSION='9.4.20-translation-stability-certification'"))failures.push('Localization runtime version is not synchronized to v9.4.19.');
