...
const release=fs.readFileSync(path.join(root,'RELEASE_VERSION.txt'),'utf8');if(!release.includes('PETATOE v9.4.5')||!release.includes('ELC_V9_4_5_LOCALIZATION_PERFORMANCE_STABILIZATION'))failures.push('Release metadata is not synchronized to v9.4.5.');
const runtime=fs.readFileSync(path.join(root,'i18n/localization-center/runtime.js'),'utf8');if(!runtime.includes("VERSION='9.4.5-localization-performance'")&&!runtime.includes("VERSION='9.4.5-localization-performance'"))failures.push('Localization runtime version is not synchronized to v9.4.5.');
