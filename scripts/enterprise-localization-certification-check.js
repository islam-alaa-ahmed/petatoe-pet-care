...
const expectedVersion=normalize(packageJson.version);
if(!/^\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/.test(expectedVersion))failures.push(`package.json contains an invalid release version: ${expectedVersion||'(empty)'}.`);
...
const normalizedReleaseIdVersion=releaseIdMatch?releaseIdMatch[1].replace(/_/g,'.'):'';
if(!releaseDisplayMatch||normalize(releaseDisplayMatch[1])!==expectedVersion)failures.push(`Release display metadata is not synchronized to package.json v${expectedVersion}.`);
if(!releaseIdMatch||normalizedReleaseIdVersion!==expectedVersion)failures.push(`Release identifier metadata is not synchronized to package.json v${expectedVersion}.`);
...
const runtimeVersion=runtimeVersionMatch?normalize(runtimeVersionMatch[1]):'';
if(!runtimeVersionMatch||!(runtimeVersion===expectedVersion||runtimeVersion.startsWith(expectedVersion+'-')))failures.push(`Localization runtime version is not synchronized to package.json v${expectedVersion}.`);
