...
if(!/^\d+\.\d+\.\d+$/.test(expectedVersion))fail('package.json release version is invalid.');
const runtime=fs.readFileSync(path.join(root,'i18n/localization-center/runtime.js'),'utf8');
if(!new RegExp("VERSION=['\"]"+expectedVersion.replace(/\./g,'\\.')+"(?:-|['\"])").test(runtime))fail('Localization Center runtime version mismatch.');
...
const expectedReleaseName=releaseLines[1]||'';
if(releaseLines[0]!==('PETATOE v'+expectedVersion)||!expectedReleaseName)fail('Release metadata mismatch.');
const index=fs.readFileSync(path.join(root,'index.html'),'utf8');
if(!index.includes("PETATOE_RELEASE_VERSION='v"+expectedVersion+"'")||!index.includes("PETATOE_RELEASE_NAME='"+expectedReleaseName+"'"))fail('index.html release metadata mismatch.');
