...
const versionManifest = JSON.parse(read('config/petatoe-version.json'));
...
requireCheck(worker.includes(`const APP_VERSION = '${versionManifest.cacheVersion}';`), 'Service Worker version lock', 'Unexpected Service Worker APP_VERSION; update the certification rule with an intentional release change.');
