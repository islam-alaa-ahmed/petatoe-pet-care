...
check(index.includes('performance/mobile-startup-loading-gate.js?v=10.0.25-sg4-6-7-router-owned-smart-reports-hydration-1'), 'HTML loads the corrected startup gate token');
check(worker.includes("const APP_VERSION = '10.0.25-sg4-6-7-router-owned-smart-reports-hydration-1';"), 'service worker namespace matches the runtime gate fix');
