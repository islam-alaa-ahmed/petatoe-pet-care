...
check(gate.includes('salesRecords:true') && !gate.includes("if(group === 'salesRecords') return false;"),'records navigation opens immediately while hydration runs in background');
...
check(manifest.runtimeContracts.recordsRouteHydration==='10.0.25-phase-d2-2-records-nonblocking-route-contract-1','records route hydration runtime contract is registered');
