...
check(ops.includes("var requestedSubTab=explicitSubTab||(navigationScreen==='appointmentsMaster'?'master':'')||pendingSubTab||'add';"),'operations preserves pending sub-route before fallback');
check(ops.includes("petatoe:appointments-intent-applied"),'operations acknowledges applied intent');
check(index.includes('10.0.25-operations-button-contract-fix-3'),'HTML assets use new runtime cache token');
check(worker.includes("const APP_VERSION = '10.0.25-operations-button-contract-fix-3';"),'service worker namespace rotated');
