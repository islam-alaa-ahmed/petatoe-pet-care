...
(function(){
  ...
  function restoreFromInput(e){var f=e&&e.target&&e.target.files&&e.target.files[0];if(!f)return;var reader=new FileReader();reader.onload=function(ev){var data=safeParseJson(ev.target.result||'{}');var check=validateBackup(data);if(!check.ok){toast(check.message);return}try{if(!confirm(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('سيتم استيراد ملف JSON للبيانات المدعومة فقط. هل تتابع؟'):'سيتم استيراد ملف JSON للبيانات المدعومة فقط. هل تتابع؟'))return;if(applyRestore(data,f.name)){toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تم الاستيراد بنجاح'):'تم الاستيراد بنجاح');render('settings','backup')}}catch(err){console.error(err);toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تعذر استيراد ملف Backup'):'تعذر استيراد ملف Backup')}};if(f.size&&f.size>MAX_BACKUP_JSON_LENGTH){toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('حجم ملف Backup أكبر من الحد الآمن'):'حجم ملف Backup أكبر من الحد الآمن');e.target.value='';return;}reader.readAsText(f);e.target.value='';}
