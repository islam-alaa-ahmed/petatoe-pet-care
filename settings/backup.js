...
(function(){
  ...
  function t(key,fallback){var c=window.PETATOE_LOCALIZATION_CENTER;return c&&typeof c.translate==='function'?c.translate('settingsBackup.backup.'+key,fallback):fallback;}
  ...
  function validateBackup(data){
    if(!data||typeof data!=='object'||Array.isArray(data))return {ok:false,message:t('invalidJson','Invalid JSON file')};
    if(data.type&&String(data.type).indexOf('PETATOE_')!==0)return {ok:false,message:t('unsupportedType','Unsupported file type')};
    if(data.records!=null&&!Array.isArray(data.records))return {ok:false,message:t('invalidRecords','Invalid sales data format')};
    if(data.users!=null&&!Array.isArray(data.users))return {ok:false,message:t('invalidUsers','Invalid users format')};
    if(data.roles!=null&&(typeof data.roles!=='object'||Array.isArray(data.roles)))return {ok:false,message:t('invalidRoles','Invalid permissions format')};
    if(data.system!=null&&(typeof data.system!=='object'||Array.isArray(data.system)))return {ok:false,message:t('invalidSystem','Invalid system settings format')};
    if(data.security!=null&&(typeof data.security!=='object'||Array.isArray(data.security)))return {ok:false,message:t('invalidSecurity','Invalid security settings format')};
    if(!Array.isArray(data.records)&&!data.users&&!data.roles&&!data.system&&!data.security&&!data.payroll)return {ok:false,message:t('emptyFile','No supported data to import')};
    return {ok:true,message:'OK'};
  }

  function renderBackupBody(){return '<div class="pet-v110-grid">'+
    '<div class="pet-v110-card"><h3>'+t('title','Backup')+'</h3><p>'+t('description','Export a Supabase JSON backup.')+'</p><div class="pet-v110-actions"><button class="pet-v110-btn green" data-v110-action="backup">'+t('exportFull','Export Backup JSON')+'</button><button class="pet-v110-btn blue" data-v110-action="data-only">'+t('exportData','Export Sales Data Only')+'</button></div></div>'+ 
    '<div class="pet-v110-card"><h3>'+t('restoreTitle','Restore')+'</h3><p>'+t('restoreDescription','Import supported data safely.')+'</p><div class="pet-v110-actions"><button class="pet-v110-btn primary" data-v110-action="pick-restore">'+t('restoreAction','Import / Restore')+'</button></div><div class="pet-v110-note">'+t('restoreHint','Export a backup before restore.')+'</div></div>'+ '</div>';}


  function exportFullBackup(){download({type:'PETATOE_FULL_BACKUP_SUPABASE',version:VERSION,createdAt:new Date().toISOString(),records:records(),users:users(),roles:roles(),system:system(),security:security(),payroll:payrollSnapshot(),treasury:treasurySnapshot(),warehouse:warehouseSnapshot(),childrenExpenses:childrenSnapshot()},'PETATOE_backup_'+new Date().toISOString().slice(0,10)+'.json');audit('Backup Exported','Supabase JSON backup','info');toast(t('exportedFull','Backup exported'));}
  function exportDataOnly(){download({type:'PETATOE_DATA_ONLY',version:VERSION,createdAt:new Date().toISOString(),records:records()},'PETATOE_records_only.json');audit('Data Exported','Records only JSON export','info');toast(t('exportedData','Data exported'));}
  ...
  function restoreFromInput(e){var f=e&&e.target&&e.target.files&&e.target.files[0];if(!f)return;var reader=new FileReader();reader.onload=function(ev){var data=safeParseJson(ev.target.result||'{}');var check=validateBackup(data);if(!check.ok){toast(check.message);return}try{if(!confirm(t('confirmRestore','Import supported JSON data?')))return;if(applyRestore(data,f.name)){toast(t('restoreSuccess','Import completed successfully'));render('settings','backup')}}catch(err){console.error(err);toast(t('restoreFailed','Unable to import backup'))}};if(f.size&&f.size>MAX_BACKUP_JSON_LENGTH){toast(t('fileTooLarge','Backup file is too large'));e.target.value='';return;}reader.readAsText(f);e.target.value='';}
