...
(function(){
  ...
  function mainTabs(active){var arr=[['system','النظام'],['settings','الإعدادات'],['setup','التهيئة'],['permissions','الصلاحيات'],['vehicleAssignment','ربط السيارات'],['users','المستخدمين']];if(window.matchMedia&&window.matchMedia('(max-width: 900px)').matches)arr.push(['about',tr('aboutApp.title','حول التطبيق')]);var obsUser=currentUser();if(isSuperUser(obsUser)||String((obsUser&&obsUser.role)||'').toLowerCase()==='admin')arr.push(['observability',tr('observability.title','الأداء والمراقبة')]);var elcUser=currentUser(), elcRole=String((elcUser&&elcUser.role)||'').toLowerCase();if(isSuperUser(elcUser)||elcRole==='admin')arr.push(['localization',tr('elc.title','مركز الترجمة')]);arr=arr.filter(function(t){return t[0]==='localization'||t[0]==='about'||settingsCanOpen(t[0])});if(!arr.length)arr=[['system','النظام']];if(!arr.some(function(t){return t[0]===active}))active=arr[0][0];return '<div class="pet-v110-main-tabs">'+arr.map(function(t){return '<button type="button" class="pet-v110-main-tab '+(active===t[0]?'active':'')+'" data-pet-v110-main="'+t[0]+'">'+t[1]+'</button>'}).join('')+'</div>'}
  ...
  function render(main,sub){
    ...
    var body=main==='settings'?subTabs(sub)+settingsBody(sub,q):(main==='setup'?setupBody():(main==='permissions'?(sub==='audit'?auditOnlyBody():permissionsBody()):(main==='vehicleAssignment'?vehicleAssignmentBody():(main==='users'?usersBody():(main==='observability'?'<div id="petatoeObservabilitySettingsMount"></div>':(main==='about'?'<div id="petatoeAboutAppMount"></div>':(main==='localization'?'<div id="petatoeLocalizationDashboardMount"></div>':systemHome(q))))))));
  ...
  window.petV110SaveSystem=function(){var v={companyName:val('v110Company','PETATOE'),currency:val('v110Currency','SAR'),monthlyTarget:Number(val('v110Target',92000))||0,vatRate:Number(val('v110Vat',15))||0,language:val('v110Lang','ar'),theme:val('v110Theme','dark')};saveSystem(v);try{document.documentElement.setAttribute('data-theme',v.theme);setText('petatoe_theme',v.theme)}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("settings/settings.js",e);}audit('System Settings Saved',v.companyName+' / '+v.currency,'info');toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تم حفظ إعدادات النظام'):'تم حفظ إعدادات النظام');render('settings','systemSettings')};
  window.petV110ClearDemo=function(){var before=records().length, kept=records().filter(function(r){var s=JSON.stringify(r).toLowerCase();return !(s.includes('demo')||s.includes('test')||s.includes('تجريبي'))});if(kept.length===before){toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('لا توجد بيانات تجريبية ظاهرة'):'لا توجد بيانات تجريبية ظاهرة');return}if(!confirm(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('حذف '):'حذف '+(before-kept.length)+' سجل تجريبي؟'))return;try{if(window.PETATOEDataSource&&window.PETATOEDataSource.setRecordsSync)window.PETATOEDataSource.setRecordsSync(kept)}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("settings/settings.js",e);}try{if(typeof save==='function')save()}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("settings/settings.js",e);}audit('Demo Data Cleared','Removed '+(before-kept.length),'warn');__qualityCache=null;toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تم مسح البيانات التجريبية'):'تم مسح البيانات التجريبية');render('settings','data')};
  window.petV110ClearSection=function(){var sec=val('v110ClearSection','');if(!sec)return;if(prompt(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('للتأكيد اكتب DELETE'):'للتأكيد اكتب DELETE')!=='DELETE'){toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تم الإلغاء'):'تم الإلغاء');return}var keys={treasury:['petatoe_treasury_movements','petatoe_treasury_audit','PETATOE_TREASURY_MOVEMENTS'],obligations:['petatoe_obligations','petatoe_obligations_archive'],vehicles:['petatoe_vehicles','petatoe_fleet']};if(sec==='allLocal'){if(confirm(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('مسح كل التخزين المحلي؟'):'مسح كل التخزين المحلي؟')){var st=S();if(st&&st.clearRaw)st.clearRaw()}}else{(keys[sec]||[]).forEach(function(k){removeKey(k)})}audit('Section Cleared',sec,'warn');toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تم تنفيذ التصفير'):'تم تنفيذ التصفير');render('settings','data')};
  window.petV110SaveSecurity=function(){saveSecurity({lockDelete:checked('v110LockDelete'),requireEditReason:checked('v110EditReason'),requireDeleteReason:checked('v110DeleteReason'),enableAudit:checked('v110Audit'),protectReports:checked('v110ProtectReports'),sensitiveAmount:Number(val('v110SensitiveAmount',10000))||0});audit('Security Settings Saved','Security settings updated','warn');toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تم حفظ إعدادات الأمان'):'تم حفظ إعدادات الأمان');render('settings','security')};
  ...
  window.petV110ClearAudit=function(){if(!confirm(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('مسح سجل النشاط؟'):'مسح سجل النشاط؟'))return;if(Audit.clearLogs)Audit.clearLogs();else{removeKey(AUDIT_KEY);audit('Audit Trail Reset','Audit log cleared','warn')}toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تم مسح السجل'):'تم مسح السجل');render('permissions','audit')};
  window.petV110RebuildIndexes=function(){try{setText('petatoe_index_rebuild_at',new Date().toISOString());if(typeof _invalidateSearchIndex==='function')_invalidateSearchIndex()}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("settings/settings.js",e);}audit('Indexes Rebuilt','Search/cache indexes rebuilt','info');toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تمت إعادة بناء الفهارس'):'تمت إعادة بناء الفهارس')};
  window.petV110CleanCache=function(){try{var l=byId('petatoeLoader');if(l){l.classList.add('hidden');l.style.display='none';l.style.pointerEvents='none'}var st=S();if(st&&st.clearRaw)st.clearRaw({scope:'session'});}catch(e){window.PETATOEUtils&&window.PETATOEUtils.warnSilentCatch&&window.PETATOEUtils.warnSilentCatch("settings/settings.js",e);}audit('Cache Cleaned','Session cache and loader cleaned','info');toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تم تنظيف الكاش'):'تم تنظيف الكاش')};
  ...
  window.petV664SaveVehicleAssignment=function(){var P=window.PETATOEPermissions||{}, uid=val('petV664AssignmentUser',''), us=users(), u=us.find(function(x){return x.id===uid});if(!uid||!u){toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('اختر مستخدم أولاً'):'اختر مستخدم أولاً');return}if(isSuperUser(u)){toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('Super Admin كامل الصلاحيات ومحمي'):'Super Admin كامل الصلاحيات ومحمي');return}if(!P.getUserPerm||!P.saveUserPerm){toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('وحدة الصلاحيات غير جاهزة'):'وحدة الصلاحيات غير جاهزة');return}var p=P.getUserPerm(uid)||{};var all=!!(byId('petV664AllVehicles')&&byId('petV664AllVehicles').checked);var list=[];document.querySelectorAll('#settings [data-v664-vehicle]').forEach(function(c){if(c.checked)list.push(c.getAttribute('data-v664-vehicle'))});p.vehicleScope={allVehicles:all,vehicles:all?[]:list};P.saveUserPerm(uid,p);audit('Vehicle Assignment Updated','Vehicle scope saved for '+(u.username||uid),'warn');toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تم حفظ ربط السيارات للمستخدم'):'تم حفظ ربط السيارات للمستخدم');render('vehicleAssignment')};
  window.petV664ClearVehicleAssignment=function(){var all=byId('petV664AllVehicles');if(all){all.checked=true;window.petV664ToggleAllVehicles(true)}toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تم تحديد كل السيارات، اضغط حفظ للتأكيد'):'تم تحديد كل السيارات، اضغط حفظ للتأكيد')};
  window.petV664CopyVehicleAssignment=function(){var P=window.PETATOEPermissions||{}, from=val('petV664CopyFrom',''), to=val('petV664CopyTo',''), us=users(), target=us.find(function(x){return x.id===to});if(!from||!to){toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('اختر مستخدم المصدر والهدف'):'اختر مستخدم المصدر والهدف');return}if(from===to){toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('لا يمكن النسخ لنفس المستخدم'):'لا يمكن النسخ لنفس المستخدم');return}if(!target){toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('المستخدم الهدف غير موجود'):'المستخدم الهدف غير موجود');return}if(isSuperUser(target)){toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('Super Admin محمي ولا يحتاج تخصيص'):'Super Admin محمي ولا يحتاج تخصيص');return}if(!P.getUserPerm||!P.saveUserPerm){toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('وحدة الصلاحيات غير جاهزة'):'وحدة الصلاحيات غير جاهزة');return}var sourceScope=(P.getVehicleScope?P.getVehicleScope(from):(P.getUserPerm(from)||{}).vehicleScope)||{allVehicles:true,vehicles:[]};var p=P.getUserPerm(to)||{};p.vehicleScope={allVehicles:sourceScope.allVehicles!==false,vehicles:(sourceScope.vehicles||[]).slice()};P.saveUserPerm(to,p);window.__PETATOE_V664_ASSIGNMENT_USER__=to;audit('Vehicle Assignment Copied','Vehicle scope copied from '+from+' to '+to,'warn');toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تم نسخ ربط السيارات'):'تم نسخ ربط السيارات');render('vehicleAssignment')};
  ...
  document.addEventListener('click',function(e){var m=e.target.closest&&e.target.closest('[data-pet-v110-main]');if(m){var main=m.getAttribute('data-pet-v110-main'),sub=(main==='settings'?(m.getAttribute('data-pet-v110-sub')||getText(SUB_KEY,'backup')||'backup'):'');if(main!=='about'&&!settingsCanOpen(main)){toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('غير متاح للصلاحية الحالية'):'غير متاح للصلاحية الحالية');return}setText(MAIN_KEY,main);if(sub)setText(SUB_KEY,sub);render(main,sub);return}var s=e.target.closest&&e.target.closest('[data-pet-v110-sub]');if(s){var sub=s.getAttribute('data-pet-v110-sub');setText(MAIN_KEY,'settings');setText(SUB_KEY,sub);render('settings',sub);return}var b=e.target.closest&&e.target.closest('[data-tab="settings"]');if(b)setTimeout(function(){render(getText(MAIN_KEY,'system')||'system',getText(SUB_KEY,'')||'')},320)},true);
  hook();
...
(function(){
  ...
  window.petV9RevokeTrustedDevice = async function(deviceId){
    if(!deviceId) return;
    if(!confirm(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('إلغاء الثقة من هذا الجهاز؟ سيُطلب OTP مرة أخرى عند الدخول منه.'):'إلغاء الثقة من هذا الجهاز؟ سيُطلب OTP مرة أخرى عند الدخول منه.')) return;
    ...
    try{
      ...
      if(!res || res.ok === false) throw new Error(errorMessage(res));
      if(window.PETATOEUI && typeof window.PETATOEUI.toast === 'function') window.PETATOEUI.toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تم إلغاء الثقة من الجهاز'):'تم إلغاء الثقة من الجهاز');
  ...
  window.petV9RevokeActiveSession = async function(sessionId){
    ...
    if(!sessionId) return;
    if(!confirm(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('إنهاء هذه الجلسة؟ سيتم إجبار هذا الجهاز على تسجيل الدخول مرة أخرى.'):'إنهاء هذه الجلسة؟ سيتم إجبار هذا الجهاز على تسجيل الدخول مرة أخرى.')) return;
    try{
      ...
      if(!res || res.ok === false) throw new Error(sessionErrorMessage(res));
      if(window.PETATOEUI && typeof window.PETATOEUI.toast === 'function') window.PETATOEUI.toast(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('تم إنهاء الجلسة'):'تم إنهاء الجلسة');
  ...
  window.petV9RevokeAllActiveSessions = async function(){
    if(!confirm(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('إنهاء كل الجلسات الأخرى؟ ستبقى هذه الجلسة الحالية فقط مفتوحة.'):'إنهاء كل الجلسات الأخرى؟ ستبقى هذه الجلسة الحالية فقط مفتوحة.')) return;
    try{
