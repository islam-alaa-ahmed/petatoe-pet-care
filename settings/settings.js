...
(function(){
  ...
  function mainTabs(active){var arr=[['system','النظام'],['settings','الإعدادات'],['setup','التهيئة'],['permissions','الصلاحيات'],['vehicleAssignment','ربط السيارات'],['users','المستخدمين']].filter(function(t){return settingsCanOpen(t[0])});if(!arr.length)arr=[['system','النظام']];if(!arr.some(function(t){return t[0]===active}))active=arr[0][0];return '<div class="pet-v110-main-tabs">'+arr.map(function(t){return '<button type="button" class="pet-v110-main-tab '+(active===t[0]?'active':'')+'" data-pet-v110-main="'+t[0]+'">'+t[1]+'</button>'}).join('')+'</div>'}
