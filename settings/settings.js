...
(function(){
  ...
  function users(){seed();var u=read(USERS_KEY,[]);var sec=window.PETATOEPasswordSecurity;if(sec&&sec.sanitizeUsers&&sec.sanitizeUsers(u))write(USERS_KEY,u);return u} function saveUsers(v){var sec=window.PETATOEPasswordSecurity;if(sec&&sec.saveUsers){sec.saveUsers(v||[]);return}write(USERS_KEY,v)}
  ...
  function mainTabs(active){var arr=[['system','النظام'],['settings','الإعدادات'],['setup','التهيئة'],['permissions','الصلاحيات'],['vehicleAssignment','ربط السيارات'],['users','المستخدمين']];return '<div class="pet-v110-main-tabs">'+arr.map(function(t){return '<button type="button" class="pet-v110-main-tab '+(active===t[0]?'active':'')+'" data-pet-v110-main="'+t[0]+'">'+t[1]+'</button>'}).join('')+'</div>'}
  ...
  function systemHome(q){return '<div class="pet-v110-grid three">'+card('🧭 ملخص النظام','نظرة عامة على حالة البرنامج والبيانات.', '<div class="pet-v110-actions"><span class="pet-v110-badge info">الفواتير: '+q.count+'</span><span class="pet-v110-badge '+(q.rows.length?'warn':'ok')+'">مشاكل البيانات: '+q.rows.length+'</span><span class="pet-v110-badge role">المستخدم: '+esc(currentUser().username)+'</span></div>')+card('⚙️ اختصارات الإعدادات','انتقل مباشرة للتبويب المطلوب.', '<div class="pet-v110-actions"><button class="pet-v110-btn primary" data-pet-v110-main="settings">الإعدادات</button><button class="pet-v110-btn blue" data-pet-v110-main="setup">التهيئة</button><button class="pet-v110-btn blue" data-pet-v110-main="permissions">الصلاحيات</button><button class="pet-v110-btn green" data-pet-v110-main="users">المستخدمين</button></div>')+card('🛡️ الحوكمة','الأمان وسجل النشاط وتجهيز النظام للتحويل لاحقًا إلى PHP وقاعدة بيانات.', '<div class="pet-v110-note">النظام الحالي يعتمد على Supabase لجداول Users / Roles / Permissions / Audit Logs.</div>')+'</div>'}
  ...
  function render(main,sub){
    ...
    var body=main==='settings'?subTabs(sub)+settingsBody(sub,q):(main==='setup'?setupBody():(main==='permissions'?(sub==='audit'?auditOnlyBody():permissionsBody()):(main==='vehicleAssignment'?vehicleAssignmentBody():(main==='users'?usersBody():systemHome(q)))));
  ...
  document.addEventListener('click',function(e){var m=e.target.closest&&e.target.closest('[data-pet-v110-main]');if(m){var main=m.getAttribute('data-pet-v110-main'),sub=(main==='settings'?(m.getAttribute('data-pet-v110-sub')||getText(SUB_KEY,'backup')||'backup'):'');setText(MAIN_KEY,main);if(sub)setText(SUB_KEY,sub);render(main,sub);return}var s=e.target.closest&&e.target.closest('[data-pet-v110-sub]');if(s){var sub=s.getAttribute('data-pet-v110-sub');setText(MAIN_KEY,'settings');setText(SUB_KEY,sub);render('settings',sub);return}var b=e.target.closest&&e.target.closest('[data-tab="settings"]');if(b)setTimeout(function(){render(getText(MAIN_KEY,'system')||'system',getText(SUB_KEY,'')||'')},320)},true);
  hook();
