...
function renderSmartReports(){
  ...
  const customerCompareExportBtn=function(kind){return `<button type="button" class="exp-btn exp-btn-excel customer-yoy-export-btn" data-smart-action="customer-compare-export" data-kind="${kind}">⬇️ Excel</button>`};
  ...
  window.PETATOECustomerCompareExportData={
    meta:{baseYear:customerCompareBaseYear,targetYear:customerCompareTargetYear,periodMode:customerComparePeriodMode,periodModeLabel:customerComparePeriodModeLabel,car:customerCompareCarFilter==='all'?'كل السيارات':customerCompareCarFilter,valueLabel:customerCompareValueLabel(),latestInvoice:customerCompareLatestRecord?String(customerCompareLatestRecord.invoice||'—'):'—'},
  ...
  const contractReasonHtml=(x,idx)=>{
    ...
    const summary=(x.contractScore||0)>=80?'إنفاق قوي + نشاط منتظم':((x.contractScore||0)>=60?'نشاط جيد + فرصة توريد دوري':'فرصة متابعة قبل التعاقد');
    ...
    <div class="smart-tab-section" data-smart-section="recommendations">
      <div class="smart-panel">
        ...
        <div class="smart-rec-pro-toolbar">
          ...
          <div class="smart-rec-summary"><span>عدد توصيات القسم:</span><b id="smartRecVisibleCount">${smartRecommendations.filter(r=>r.cat==='revenue').length}</b><span>مبنية على:</span><b>${fmt0(count)} عملية</b></div>
        </div>
