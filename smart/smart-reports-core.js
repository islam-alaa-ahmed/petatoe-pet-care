...
function smartFormatDate(value,options){
  try{
    var center=window.PETATOE_LOCALIZATION_CENTER;
    if(center&&typeof center.formatDate==='function') return center.formatDate(value,options||{});
  }catch(_){ }
  return fmtDateAr(value);
}
function smartListJoin(values){
  var separator=window.PETATOE_LOCALIZATION_CENTER.smart('format.listSeparator','، ');
...
function smartReportMonth(monthName, fallback){
  ...
  return window.PETATOE_LOCALIZATION_CENTER.smart('calendar.months.'+normalized, fallback==null?(MAR[normalized]||raw):fallback);
}
...
function renderSmartReports(){
  ...
  const overviewCardsYearLabel=overviewCardsSelected==='all'?window.PETATOE_LOCALIZATION_CENTER.smart('filters.allYears','كل السنوات'):String(overviewCardsSelected);
  ...
  const aiServiceRiskRows=(aiForecast.serviceRisk||[]).map(x=>`<div class="ai-risk-row"><div><strong>${htmlSafe(smartServiceDisplay(x.name))}</strong><small>${smartReportHtml('labels.estimatedDecline','تراجع تقديري')} ${fmt(Math.abs(x.change))}%</small></div><span class="risk-pill bad">${smartReportHtml('status.declineRisk','خطر هبوط')}</span></div>`).join('')||`<div class="ai-risk-row"><div><strong>${smartReportHtml('empty.noCriticalServices','لا توجد خدمات حرجة')}</strong><small>${smartReportHtml('empty.noClearServiceDecline','لم يظهر هبوط واضح في الخدمات الحالية')}</small></div><span class="risk-pill good">${smartReportHtml('status.reassuring','مطمئن')}</span></div>`;
  const aiServiceGrowthRows=(aiForecast.serviceGrowth||[]).map(x=>`<div class="ai-risk-row"><div><strong>${htmlSafe(smartServiceDisplay(x.name))}</strong><small>${smartReportHtml('labels.estimatedGrowth','نمو تقديري')} ${fmt(x.change)}%</small></div><span class="risk-pill good">${smartReportHtml('status.growthCandidate','مرشحة للنمو')}</span></div>`).join('')||`<div class="ai-risk-row"><div><strong>${smartReportHtml('empty.noExceptionalGrowth','لا يوجد نمو استثنائي')}</strong><small>${smartReportHtml('empty.balancedActivity','النشاط موزع بشكل متوازن')}</small></div><span class="risk-pill info">${smartReportHtml('status.neutral','محايد')}</span></div>`;
  ...
  const newReturningClientRows=Object.entries(clientCounts).map(([name,cnt])=>{
    ...
    return {name,operations:cnt,invoices:invoices.length,totalValue,first,last,lastInvoice:String((sorted[sorted.length-1]||{}).invoice||'—'),vans:smartListJoin(vans),pays:smartListJoin(pays)};
  ...
  const salesTargetStatus=!salesTarget?window.PETATOE_LOCALIZATION_CENTER.smart('status.notSpecified','غير محدد'):(salesTargetActual>=salesTarget?window.PETATOE_LOCALIZATION_CENTER.smart('status.targetAchieved','تم تحقيق الهدف'):window.PETATOE_LOCALIZATION_CENTER.smart('status.targetNotAchieved','لم يكتمل الهدف'));
  ...
  const salesAlerts=[];
  const lastMonthLabel=salesLastMonth.m&&salesLastMonth.m!=='-'?window.PETATOE_LOCALIZATION_CENTER.smart('calendar.months.'+salesLastMonth.m,MAR[salesLastMonth.m]):window.PETATOE_LOCALIZATION_CENTER.smart('alerts.lastActiveMonth','آخر شهر نشط');
  const prevMonthLabel=salesPrevMonth.m&&salesPrevMonth.m!=='-'?window.PETATOE_LOCALIZATION_CENTER.smart('calendar.months.'+salesPrevMonth.m,MAR[salesPrevMonth.m]):window.PETATOE_LOCALIZATION_CENTER.smart('alerts.previousMonth','الشهر السابق');
  ...
  salesAlerts.push({type:salesMonthDiff>=0?'good':'bad',text:window.PETATOE_LOCALIZATION_CENTER.smart(salesMonthDiff>=0?'alerts.monthIncreaseText':'alerts.monthDecreaseText',salesMonthDiff>=0?'ارتفاع آخر شهر نشط عن الشهر السابق بقيمة {value} ({percent}%).':'انخفاض آخر شهر نشط عن الشهر السابق بقيمة {value} ({percent}%).',{value:money(Math.abs(salesMonthDiff)),percent:salesMonthDiffPct.toFixed(1)}),detail:monthTrendDetail});
  if(bestEffVan[0]&&bestEffVan[0]!=='-'){
    const vanRank=vanAvgRev.findIndex(v=>v[0]===bestEffVan[0])+1;
    const vanDetail=`<b>${smartReportHtml('alerts.vehicleEfficiencyDetails','تفاصيل كفاءة السيارة')}</b><div class="tip-grid">${alertDetailBox(smartReportHtml('overview.vehicle','السيارة'),htmlSafe(bestEffVan[0]))}${alertDetailBox(smartReportHtml('table.rank','الترتيب'),`#${fmt0(vanRank||1)} ${smartReportHtml('common.of','من')} ${fmt0(vanAvgRev.length)}`)}${alertDetailBox(smartReportHtml('overview.totalSales','إجمالي المبيعات'),money(bestEffVan[1]||0))}${alertDetailBox(smartReportHtml('vehicleEfficiency.operationsCount','عدد العمليات'),fmt0(bestEffVan[2]||0))}${alertDetailBox(smartReportHtml('overview.averageTransaction','متوسط العملية'),money(bestEffVan[3]||0))}${alertDetailBox(smartReportHtml('vehicleEfficiency.contribution','المساهمة'),safeDiv(bestEffVan[1]||0,total).toLocaleString('en-US',{style:'percent',maximumFractionDigits:1}))}</div><span class="ok">${smartReportHtml('alerts.vehicleSelectionReason','✓ تم اختيار السيارة بناءً على أعلى متوسط إيراد لكل عملية.')}</span><span>${smartReportHtml('alerts.vehicleRecommendation','التوصية: استخدم نمط تشغيل هذه السيارة كمرجع لباقي السيارات الأقل كفاءة.')}</span>`;
    salesAlerts.push({type:'good',text:window.PETATOE_LOCALIZATION_CENTER.smart('alerts.bestVehicleText','السيارة الأعلى كفاءة هي {vehicle} بمتوسط {average} لكل عملية.',{vehicle:bestEffVan[0],average:money(bestEffVan[3]||0)}),detail:vanDetail});
  }
  if(salesAtRiskRows.length){
    const riskRows=salesAtRiskRows.slice(0,8).map((r,i)=>`<tr><td>${i+1}</td><td>${htmlSafe(r.name)}</td><td>${r.last?smartFormatDate(r.last):'—'}</td><td>${fmt0(r.days)} ${smartReportHtml('units.day','يوم')}</td><td>${money(r.value)}</td></tr>`).join('');
    ...
    salesAlerts.push({type:'warn',text:window.PETATOE_LOCALIZATION_CENTER.smart('alerts.atRiskCountText','يوجد {count} عميل من كبار العملاء لم يزوروا منذ أكثر من 60 يوم.',{count:fmt0(salesAtRiskRows.length)}),detail:riskDetail});
  }
  if(topService[0]&&topService[0]!=='-'){
    ...
    const topServiceDetail=`<b>${smartReportHtml('alerts.topServiceDetails','تفاصيل أعلى خدمة قيمة')}</b><div class="tip-grid">${alertDetailBox(smartReportHtml('table.service','الخدمة'),htmlSafe(smartServiceDisplay(topService[0])))}${alertDetailBox(smartReportHtml('overview.totalSales','إجمالي المبيعات'),money(topService[1]||0))}${alertDetailBox(smartReportHtml('vehicleEfficiency.operationsCount','عدد العمليات'),fmt0(topServiceOps))}${alertDetailBox(smartReportHtml('overview.averageTransaction','متوسط العملية'),money(topServiceAvg))}${alertDetailBox(smartReportHtml('alerts.serviceContribution','مساهمة الخدمة'),safeDiv(topService[1]||0,total).toLocaleString('en-US',{style:'percent',maximumFractionDigits:1}))}${alertDetailBox(smartReportHtml('alerts.top5ServicesContribution','مساهمة أعلى 5 خدمات'),`${serviceTop5Share.toFixed(1)}%`)}</div><span class="ok">${smartReportHtml('alerts.topServiceReason','✓ الخدمة الأعلى قيمة تم تحديدها من إجمالي مبيعات الخدمة في الفترة المختارة.')}</span><span>${smartReportHtml('alerts.topServiceRecommendation','التوصية: اعمل Bundle أو عرض مكمل لهذه الخدمة لرفع متوسط الفاتورة.')}</span>`;
    salesAlerts.push({type:'good',text:window.PETATOE_LOCALIZATION_CENTER.smart('alerts.topServiceText','الخدمة الأعلى قيمة هي {service} بإجمالي {total}.',{service:smartServiceDisplay(topService[0]),total:money(topService[1])}),detail:topServiceDetail});
  }

  const dayNames=['sunday','monday','tuesday','wednesday','thursday','friday','saturday'].map(k=>window.PETATOE_LOCALIZATION_CENTER.smart('calendar.days.'+k,k));
  ...
  const heatMonthPalettes=[
    {name:window.PETATOE_LOCALIZATION_CENTER.smart('calendar.months.January','يناير'),   light:'#9ed8ff', dark:'#2458d6'},
    {name:window.PETATOE_LOCALIZATION_CENTER.smart('calendar.months.February','فبراير'), light:'#92c9ff', dark:'#1d74c9'},
    {name:window.PETATOE_LOCALIZATION_CENTER.smart('calendar.months.March','مارس'),   light:'#78e5df', dark:'#0b9ea6'},
    {name:window.PETATOE_LOCALIZATION_CENTER.smart('calendar.months.April','أبريل'),  light:'#83e0b1', dark:'#15985a'},
    {name:window.PETATOE_LOCALIZATION_CENTER.smart('calendar.months.May','مايو'),   light:'#c2e674', dark:'#73a728'},
    {name:window.PETATOE_LOCALIZATION_CENTER.smart('calendar.months.June','يونيو'),  light:'#ffe174', dark:'#d6a514'},
    {name:window.PETATOE_LOCALIZATION_CENTER.smart('calendar.months.July','يوليو'),  light:'#ffc06c', dark:'#e56f18'},
    {name:window.PETATOE_LOCALIZATION_CENTER.smart('calendar.months.August','أغسطس'), light:'#ff9b65', dark:'#d9492c'},
    {name:window.PETATOE_LOCALIZATION_CENTER.smart('calendar.months.September','سبتمبر'),light:'#ff91ad', dark:'#c43763'},
    {name:window.PETATOE_LOCALIZATION_CENTER.smart('calendar.months.October','أكتوبر'), light:'#d596f1', dark:'#873ab6'},
    {name:window.PETATOE_LOCALIZATION_CENTER.smart('calendar.months.November','نوفمبر'), light:'#b798ff', dark:'#5d3fb7'},
    {name:window.PETATOE_LOCALIZATION_CENTER.smart('calendar.months.December','ديسمبر'),light:'#89aaf7', dark:'#2754b8'}
  ];
  ...
  const customerValueTierHtml=(cls,badgeClass,stats)=>{
    ...
    const title=window.PETATOE_LOCALIZATION_CENTER.smart(titleKey,cls==='VIP'?'VIP':cls==='At Risk'?'معرض للخطر':'نشط');
    const reasons=[];
    if(cls==='VIP'){
      reasons.push(window.PETATOE_LOCALIZATION_CENTER.smart('classification.reason.vipVisits','عدد الزيارات {visits} زيارة، وهو أكبر من أو يساوي 5 زيارات',{visits:fmt0(stats.visits)}));
      reasons.push(window.PETATOE_LOCALIZATION_CENTER.smart('classification.reason.vipSpend','إجمالي الإنفاق {value} يساوي أو يتجاوز متوسط النشاط المتوقع لهذا العميل',{value:money(stats.value)}));
      reasons.push(window.PETATOE_LOCALIZATION_CENTER.smart('classification.reason.vipRecency','آخر زيارة منذ {days} يوم، أي خلال آخر 45 يوم',{days:fmt0(stats.daysSince)}));
    }else if(cls==='At Risk'){
      if(stats.daysSince>60) reasons.push(window.PETATOE_LOCALIZATION_CENTER.smart('classification.reason.riskAbsence','آخر زيارة منذ {days} يوم، أي أكثر من 60 يوم',{days:fmt0(stats.daysSince)}));
      if(stats.visits<=1) reasons.push(window.PETATOE_LOCALIZATION_CENTER.smart('classification.reason.riskLowVisits','عدد الزيارات {visits} فقط، لذلك العميل لم يظهر عليه تكرار كافٍ',{visits:fmt0(stats.visits)}));
      if(stats.daysSince<=60 && stats.visits>1) reasons.push(window.PETATOE_LOCALIZATION_CENTER.smart('classification.reason.riskFollowup','العميل يحتاج متابعة لأنه خارج شروط VIP / Active المثالية'));
    }else{
      reasons.push(window.PETATOE_LOCALIZATION_CENTER.smart('classification.reason.activeRecency','آخر زيارة منذ {days} يوم، لذلك العميل ما زال نشطًا',{days:fmt0(stats.daysSince)}));
      reasons.push(window.PETATOE_LOCALIZATION_CENTER.smart('classification.reason.activeVisitsSpend','عدد الزيارات {visits} زيارة بإجمالي إنفاق {value}',{visits:fmt0(stats.visits),value:money(stats.value)}));
      reasons.push(window.PETATOE_LOCALIZATION_CENTER.smart('classification.reason.activeConditions','العميل ليس ضمن At Risk، ولم يحقق كل شروط VIP الحالية'));
    }
    const tooltipTitle=window.PETATOE_LOCALIZATION_CENTER.smart('classification.reasonTitle','سبب تصنيف العميل: {classification}',{classification:title});
    const tooltipIntro=window.PETATOE_LOCALIZATION_CENTER.smart('classification.reasonIntro','تم تصنيف العميل بناءً على بيانات الفترة المختارة:');
    const tooltipNote=window.PETATOE_LOCALIZATION_CENTER.smart('classification.reasonNote','المعادلة تستخدم نفس القيم الظاهرة في الجدول بدون تعديل أي رقم أو إجمالي.');
  ...
  const allCustomerValueRows=customerReportNames.map(name=>{
    ...
    const risk=window.PETATOE_LOCALIZATION_CENTER.smart('risk.'+riskKey,({low:'منخفض',medium:'متوسط',high:'مرتفع',critical:'حرج'})[riskKey]);
    const recovery=window.PETATOE_LOCALIZATION_CENTER.smart('recovery.'+recoveryKey,({low:'منخفضة',medium:'متوسطة',high:'مرتفعة'})[recoveryKey]);
  ...
  const customerInsightRows=allCustomerValueRows.slice(0,customerInsightLimit).map(x=>
    `<tr><td>${htmlSafe(x.name)}</td><td>${money(x.val)}</td><td>${x.visitsHtml}</td><td>${money(x.avgInvoice)}</td><td>${x.lastDate?smartFormatDate(x.lastDate):'—'}</td><td>${money(x.lastInvoiceValue)}</td><td>${x.visitMonthsHtml}</td><td>${fmt0(x.daysSince)}</td><td>${x.tierHtml}</td></tr>`
  ...
  const customerInsightMoreButton=allCustomerValueRows.length>customerInsightLimit
    ? `<div class="new-cust-table-footer"><button class="new-cust-more-btn" data-smart-action="customer-insight-more" data-limit="${customerInsightLimit+10}">${smartReportHtml('common.loadMore','اضغط لعرض المزيد ⌄')}</button><span>${smartReportHtml('common.showingCustomers','عرض {shown} من أصل {total} عميل',{shown:fmt0(Math.min(customerInsightLimit,allCustomerValueRows.length)),total:fmt0(allCustomerValueRows.length)})}</span></div>`
    : `<div class="new-cust-table-footer"><span>${smartReportHtml('common.shownCustomers','تم عرض {shown} من أصل {total} عميل',{shown:fmt0(allCustomerValueRows.length),total:fmt0(allCustomerValueRows.length)})}</span></div>`;
  ...
  const customerCompareValueLabel=function(){return customerCompareTaxMode==='net'?window.PETATOE_LOCALIZATION_CENTER.smart('metrics.net','قبل الضريبة'):(customerCompareTaxMode==='tax'?window.PETATOE_LOCALIZATION_CENTER.smart('metrics.vat','الضريبة'):window.PETATOE_LOCALIZATION_CENTER.smart('metrics.gross','شامل الضريبة'))};
  ...
  const customerComparePeriodModeLabel=customerCompareUseFullYear?window.PETATOE_LOCALIZATION_CENTER.smart('period.fullYear','السنة كاملة'):window.PETATOE_LOCALIZATION_CENTER.smart('period.currentPeriod','حتى الفترة الحالية');
  const customerComparePeriodLabel=customerCompareUseFullYear
    ? window.PETATOE_LOCALIZATION_CENTER.smart('period.fullYearComparison','وضع المقارنة: السنة كاملة — من 01 يناير إلى 31 ديسمبر {targetYear} مقابل نفس السنة كاملة {baseYear}.',{targetYear:customerCompareTargetYear,baseYear:customerCompareBaseYear})
    : (customerCompareLatestDate
      ? window.PETATOE_LOCALIZATION_CENTER.smart('period.ytdComparison','وضع المقارنة: حتى الفترة الحالية — من 01 يناير {targetYear} إلى {targetEnd} مقابل نفس الفترة من {baseYear} حتى {baseEnd} — آخر فاتورة مرفوعة: {invoice}',{targetYear:customerCompareTargetYear,targetEnd:smartFormatDate(customerCompareTargetEnd),baseYear:customerCompareBaseYear,baseEnd:smartFormatDate(customerCompareBaseEnd),invoice:htmlSafe(String(customerCompareLatestRecord.invoice||'—'))})
      : window.PETATOE_LOCALIZATION_CENTER.smart('period.noDatedInvoices','وضع المقارنة: حتى الفترة الحالية — لا توجد فواتير مؤرخة في سنة المقارنة {targetYear} ضمن الفلتر الحالي.',{targetYear:customerCompareTargetYear}));
  ...
  const customerCompareCarItems=customerCompareCars.map(c=>({value:String(c),text:c==='all'?window.PETATOE_LOCALIZATION_CENTER.smart('filters.allVehicles','كل السيارات'):String(c)}));
  const customerCompareTopItems=['10','20','50','all'].map(v=>({value:v,text:v==='all'?window.PETATOE_LOCALIZATION_CENTER.smart('filters.all','الكل'):window.PETATOE_LOCALIZATION_CENTER.smart('filters.topN','أعلى {count}',{count:v})}));
  const customerComparePeriodItems=[{value:'ytd',text:window.PETATOE_LOCALIZATION_CENTER.smart('period.currentPeriod','حتى الفترة الحالية')},{value:'full',text:window.PETATOE_LOCALIZATION_CENTER.smart('period.fullYear','السنة كاملة')}];
  ...
  const customerCompareFiltersHtml=[
    customerCompareFilterDropdown('baseYear',window.PETATOE_LOCALIZATION_CENTER.smart('compare.baseYear','سنة الأساس'),customerCompareBaseYear,customerCompareYearItems),
    customerCompareFilterDropdown('targetYear',window.PETATOE_LOCALIZATION_CENTER.smart('compare.targetYear','سنة المقارنة'),customerCompareTargetYear,customerCompareYearItems),
    customerCompareFilterDropdown('car',window.PETATOE_LOCALIZATION_CENTER.smart('compare.vehicle','السيارة'),customerCompareCarFilter,customerCompareCarItems),
    customerCompareFilterDropdown('periodMode',window.PETATOE_LOCALIZATION_CENTER.smart('compare.comparisonType','نوع المقارنة'),customerComparePeriodMode,customerComparePeriodItems),
    customerCompareFilterDropdown('top',window.PETATOE_LOCALIZATION_CENTER.smart('compare.display','عرض'),customerCompareTopFilter,customerCompareTopItems)
  ].join('');
  ...
  const customerCompareMonthNames=['January','February','March','April','May','June','July','August','September','October','November','December'].map(k=>window.PETATOE_LOCALIZATION_CENTER.smart('calendar.months.'+k,k));
  ...
  const customerCompareRows=Object.values(customerCompareMap).map(x=>{
    ...
    let status=window.PETATOE_LOCALIZATION_CENTER.smart('status.stable','ثابت'), cls='info';
    if(x.base>0 && x.target<=0){status=window.PETATOE_LOCALIZATION_CENTER.smart('status.lost','مفقود');cls='bad';}
    else if(x.base<=0 && x.target>0){status=window.PETATOE_LOCALIZATION_CENTER.smart('status.newCustomer','عميل جديد');cls='good';}
    else if(diff>0){status=window.PETATOE_LOCALIZATION_CENTER.smart('status.growth','نمو');cls='good';}
    else if(diff<0){status=window.PETATOE_LOCALIZATION_CENTER.smart('status.decline','تراجع');cls='warn';}
  ...
  const customerCompareRankHtml=function(x){return x.rankShift>0?`<span class="metric-up">${smartReportHtml('customerCompare.rankUp','⬆️ صعد {count} مركز',{count:fmt0(x.rankShift)})}</span>`:x.rankShift<0?`<span class="metric-down">${smartReportHtml('customerCompare.rankDown','⬇️ هبط {count} مركز',{count:fmt0(Math.abs(x.rankShift))})}</span>`:'—'};
  const customerCompareMore=function(key,arr,labelKey){const lim=customerCompareLimitOf(key);const unit=window.PETATOE_LOCALIZATION_CENTER.smart(labelKey,labelKey);return arr.length>lim?`<div class="new-cust-table-footer"><button class="new-cust-more-btn" data-smart-action="customer-compare-more" data-key="${key}">${smartReportHtml('common.loadMore','اضغط لعرض المزيد ⌄')}</button><span>${smartReportHtml('common.showingGeneric','عرض {shown} من أصل {total} {unit}',{shown:fmt0(Math.min(lim,arr.length)),total:fmt0(arr.length),unit:unit})}</span></div>`:`<div class="new-cust-table-footer"><span>${smartReportHtml('common.shownGeneric','تم عرض {shown} من أصل {total} {unit}',{shown:fmt0(arr.length),total:fmt0(arr.length),unit:unit})}</span></div>`};
  const customerCompareMainLimit=customerCompareLimitOf('customerCompareTableLimit');
  const customerCompareMainTable=customerCompareMainRows.slice(0,customerCompareMainLimit).map((x,i)=>`<tr><td>${i+1}</td><td>${htmlSafe(x.name)}</td><td>${money(x.base)}</td><td>${money(x.target)}</td><td class="${x.diff>=0?'metric-up':'metric-down'}">${x.diff>=0?'+':''}${money(x.diff)}</td><td>${customerComparePctHtml(x)}</td><td>${fmt0(x.baseInv)}</td><td>${fmt0(x.targetInv)}</td><td>${x.lastDate?smartFormatDate(x.lastDate):'—'}</td><td>${customerCompareRankHtml(x)}</td><td><span class="smart-tag ${x.cls}">${x.status}</span></td></tr>`).join('') || `<tr><td colspan="11">${smartReportHtml('empty.noYearComparisonData','لا توجد بيانات كافية للمقارنة بين السنتين المختارتين.')}</td></tr>`;
  const customerCompareAllDetailedTableHtml=(function(){
    ...
    return window.PETATOETables.render({
      ...
      columns:[
        ...
        {label:window.PETATOE_LOCALIZATION_CENTER.smart('table.customer','العميل'), render:function(x){return htmlSafe(x.name);}},
        ...
        {label:window.PETATOE_LOCALIZATION_CENTER.smart('table.difference','الفرق'), render:function(x){return `<span class="${x.diff>=0?'metric-up':'metric-down'}">${x.diff>=0?'+':''}${money(x.diff)}</span>`;}},
        {label:window.PETATOE_LOCALIZATION_CENTER.smart('customerCompare.growthPercent','النمو %'), render:function(x){return customerComparePctHtml(x);}},
        {label:window.PETATOE_LOCALIZATION_CENTER.smart('customerCompare.invoicesForYear','فواتير {year}',{year:customerCompareBaseYear}), render:function(x){return fmt0(x.baseInv);}},
        {label:window.PETATOE_LOCALIZATION_CENTER.smart('customerCompare.invoicesForYear','فواتير {year}',{year:customerCompareTargetYear}), render:function(x){return fmt0(x.targetInv);}},
        {label:window.PETATOE_LOCALIZATION_CENTER.smart('table.lastInteraction','آخر تعامل'), render:function(x){return x.lastDate?smartFormatDate(x.lastDate):'—';}},
        {label:window.PETATOE_LOCALIZATION_CENTER.smart('customerCompare.rankChange','تغير الترتيب'), render:function(x){return customerCompareRankHtml(x);}},
        {label:window.PETATOE_LOCALIZATION_CENTER.smart('table.status','الحالة'), render:function(x){return `<span class="smart-tag ${x.cls}">${x.status}</span>`;}}
      ]
    });
  })();
  ...
  const customerCompareLostRiskMeta=function(v){
    if(v>=20000) return {label:window.PETATOE_LOCALIZATION_CENTER.smart('risk.high','مرتفع'), cls:'bad', icon:'🔴'};
    if(v>=5000) return {label:window.PETATOE_LOCALIZATION_CENTER.smart('risk.medium','متوسط'), cls:'warn', icon:'🟡'};
    return {label:window.PETATOE_LOCALIZATION_CENTER.smart('risk.low','منخفض'), cls:'good', icon:'🟢'};
  };
  const customerCompareLostDetailsButton=function(x){
    ...
    const payload={
      ...
      lastVisit:x.baseLast?smartFormatDate(x.baseLast):'—',
  ...
  const customerCompareLostSimpleTableHtml=customerCompareRenderVirtualTable('customerCompareLostSimpleVirtualTable',customerCompareLostRows,customerCompareLostLimit,[
    ...
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('table.customer','العميل'),render:(x)=>htmlSafe(x.name)},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('table.lastInvoice','آخر فاتورة'),render:(x)=>x.baseLast?smartFormatDate(x.baseLast):'—'},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('table.lastInvoiceNumber','رقم آخر فاتورة'),render:(x)=>htmlSafe(x.baseLastInvoice||'—')},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('customerCompare.valueForYear','قيمة {year}',{year:customerCompareBaseYear}),render:(x)=>money(x.base)}
  ],customerCompareLostSimpleFallbackTable);
  const customerCompareLostFallbackTable=`<div class="smart-table-clean customer-yoy-table"><table><thead><tr><th>#</th><th>${smartReportHtml('table.customer','العميل')}</th><th>${smartReportHtml('customerCompare.previousYearSales','مبيعات السنة السابقة')}</th><th>${smartReportHtml('customerCompare.invoiceCount','عدد الفواتير')}</th><th>${smartReportHtml('customerCompare.lastInvoice','آخر فاتورة')}</th><th>${smartReportHtml('table.lastVisit','آخر زيارة')}</th><th>${smartReportHtml('customerCompare.lostValue','القيمة المفقودة')}</th><th>${smartReportHtml('customerCompare.riskClassification','تصنيف الخطورة')}</th><th>${smartReportHtml('actions.viewDetails','عرض التفاصيل')}</th></tr></thead><tbody>${customerCompareLostRows.slice(0,customerCompareLostLimit).map((x,i)=>{const risk=customerCompareLostRiskMeta(x.base||0);return `<tr><td>${i+1}</td><td><b>${htmlSafe(x.name)}</b></td><td>${money(x.base)}</td><td>${fmt0(x.baseInv)}</td><td>${htmlSafe(x.baseLastInvoice||'—')}</td><td>${x.baseLast?smartFormatDate(x.baseLast):'—'}</td><td>${money(x.base)}</td><td><span class="smart-tag ${risk.cls}">${risk.icon} ${risk.label}</span></td><td>${customerCompareLostDetailsButton(x)}</td></tr>`}).join('') || `<tr><td colspan="9">${smartReportHtml('customerCompare.noLostRows','لا توجد عملاء ظهروا في سنة الأساس ولم يظهروا في سنة المقارنة.')}</td></tr>`}</tbody></table></div>`;
  const customerCompareLostTableHtml=customerCompareRenderVirtualTable('customerCompareLostDetailedVirtualTable',customerCompareLostRows,customerCompareLostLimit,[
    {label:'#',render:(x)=>String((customerCompareLostRows.indexOf(x)||0)+1)},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('table.customer','العميل'),render:(x)=>`<b>${htmlSafe(x.name)}</b>`},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('customerCompare.previousYearSales','مبيعات السنة السابقة'),render:(x)=>money(x.base)},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('customerCompare.invoiceCount','عدد الفواتير'),render:(x)=>fmt0(x.baseInv)},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('customerCompare.lastInvoice','آخر فاتورة'),render:(x)=>htmlSafe(x.baseLastInvoice||'—')},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('table.lastVisit','آخر زيارة'),render:(x)=>x.baseLast?smartFormatDate(x.baseLast):'—'},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('customerCompare.lostValue','القيمة المفقودة'),render:(x)=>money(x.base)},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('customerCompare.riskClassification','تصنيف الخطورة'),render:(x)=>{const risk=customerCompareLostRiskMeta(x.base||0);return `<span class="smart-tag ${risk.cls}">${risk.icon} ${risk.label}</span>`}},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('actions.viewDetails','عرض التفاصيل'),render:(x)=>customerCompareLostDetailsButton(x)}
  ...
  const customerCompareRankTableHtml=customerCompareRenderVirtualTable('customerCompareRankShiftVirtualTable',customerCompareRankRows,customerCompareRankLimit,[
    ...
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('table.customer','العميل'),render:(x)=>htmlSafe(x.name)},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('customerCompare.rankForYear','ترتيب {year}',{year:customerCompareBaseYear}),render:(x)=>'#'+fmt0(x.baseRank)},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('customerCompare.rankForYear','ترتيب {year}',{year:customerCompareTargetYear}),render:(x)=>'#'+fmt0(x.targetRank)},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('customerCompare.change','التغير'),render:(x)=>customerCompareRankHtml(x)},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('customerCompare.salesDifference','فرق المبيعات'),render:(x)=>money(x.diff)}
  ...
  const customerCompareDateText=function(d){return d?smartFormatDate(d):'—'};
  window.PETATOECustomerCompareExportData={
    meta:{baseYear:customerCompareBaseYear,targetYear:customerCompareTargetYear,periodMode:customerComparePeriodMode,periodModeLabel:customerComparePeriodModeLabel,car:customerCompareCarFilter==='all'?window.PETATOE_LOCALIZATION_CENTER.smart('filters.allVehicles','كل السيارات'):customerCompareCarFilter,valueLabel:customerCompareValueLabel(),latestInvoice:customerCompareLatestRecord?String(customerCompareLatestRecord.invoice||'—'):'—'},
    ...
    all:{title:window.PETATOE_LOCALIZATION_CENTER.smart('export.allCustomersDetailed','كل العملاء - مقارنة تفصيلية'),sheetName:'Detailed Compare',fileName:'Customer_Compare_All_Customers_Detailed',headers:['#','العميل',String(customerCompareBaseYear),String(customerCompareTargetYear),'الفرق','النمو %','فواتير '+customerCompareBaseYear,'فواتير '+customerCompareTargetYear,'آخر تعامل','تغير الترتيب','الحالة'],rows:customerCompareMainRows.map((x,i)=>[i+1,x.name,customerCompareNum(x.base),customerCompareNum(x.target),customerCompareNum(x.diff),Number(customerCompareNum(x.pct).toFixed(2)),x.baseInv,x.targetInv,customerCompareDateText(x.lastDate),x.rankShift,x.status])}
  };
  const customerCompareTopDecline=customerCompareDeclineRows[0];
  const customerCompareInsightsHtml=`<div class="customer-yoy-insights"><h4>${smartReportHtml('customerCompare.executiveInsights','🧠 Executive Insights')}</h4><div class="insights"><div class="insight good"><b>${smartReportHtml('customerCompare.customerGrowth','نمو العملاء')}</b><span>${smartReportHtml('customerCompare.customerGrowthNarrative','ارتفع إنفاق {count} عميل مقارنة بسنة الأساس بإجمالي نمو {amount}.',{count:fmt0(customerCompareGrowth),amount:money(customerCompareGrowthRows.reduce((s,x)=>s+x.diff,0))})}</span></div><div class="insight ${customerCompareLost?'bad':'good'}"><b>${smartReportHtml('customerCompare.lostCustomers','العملاء المفقودون')}</b><span>${smartReportHtml('customerCompare.lostNarrative','{count} عميل توقفوا بالكامل في سنة المقارنة بقيمة مبيعات مفقودة {amount}{largest}.',{count:fmt0(customerCompareLost),amount:money(customerCompareLostRows.reduce((s,x)=>s+x.base,0)),largest:customerCompareTopLost?window.PETATOE_LOCALIZATION_CENTER.smart('customerCompare.largestLostSuffix','، وأكبرهم {name}',{name:customerCompareTopLost.name}):''})}</span></div><div class="insight ${customerCompareTotalDiff>=0?'good':'bad'}"><b>${smartReportHtml('customerCompare.netDifference','صافي الفرق')}</b><span>${smartReportHtml('customerCompare.netDifferenceNarrative','صافي فرق المبيعات بين فترتي {baseYear} و {targetYear} هو {amount} حسب وضع {mode}.',{baseYear:customerCompareBaseYear,targetYear:customerCompareTargetYear,amount:(customerCompareTotalDiff>=0?'+':'')+money(customerCompareTotalDiff),mode:customerCompareValueLabel()})}</span></div><div class="insight info"><b>${smartReportHtml('customerCompare.largestOpportunity','أكبر فرصة')}</b><span>${customerCompareTopGrowth?smartReportHtml('customerCompare.topGrowthNarrative','أكبر عميل نمو هو {name} بنسبة {percent}%.',{name:customerCompareTopGrowth.name,percent:customerCompareTopGrowth.pct.toFixed(1)}):(customerCompareTopDecline?smartReportHtml('customerCompare.topDeclineNarrative','أكبر تراجع يحتاج متابعة هو {name}.',{name:customerCompareTopDecline.name}):smartReportHtml('customerCompare.noGrowthOpportunity','لا توجد فرصة نمو واضحة في الفترة المختارة.'))}</span></div></div></div>`;
  const customerCompareHtml=`
      <div class="smart-panel customer-yoy-panel">
        ...
        <div class="customer-yoy-sections customer-yoy-stack">
          <div class="smart-panel"><h4>📈 Top Growth Customers ${customerCompareExportBtn('growth')}</h4><div class="smart-table-clean customer-yoy-table"><table><thead><tr><th>#</th><th>${smartReportHtml('table.customer','العميل')}</th><th>${customerCompareBaseYear}</th><th>${customerCompareTargetYear}</th><th>${smartReportHtml('table.difference','الفرق')}</th><th>${smartReportHtml('customerCompare.growthPercent','النمو %')}</th></tr></thead><tbody>${customerCompareGrowthTable}</tbody></table></div>${customerCompareMore('customerCompareGrowthLimit',customerCompareGrowthRows,'units.growthCustomer')}</div>
          <div class="smart-panel"><h4>📉 Top Declining Customers ${customerCompareExportBtn('decline')}</h4><div class="smart-table-clean customer-yoy-table"><table><thead><tr><th>#</th><th>${smartReportHtml('table.customer','العميل')}</th><th>${customerCompareBaseYear}</th><th>${customerCompareTargetYear}</th><th>${smartReportHtml('table.difference','الفرق')}</th><th>${smartReportHtml('customerCompare.declinePercent','التراجع %')}</th></tr></thead><tbody>${customerCompareDeclineTable}</tbody></table></div>${customerCompareMore('customerCompareDeclineLimit',customerCompareDeclineRows,'عميل تراجع')}</div>
        </div>
        <div class="smart-panel"><h4>${smartReportHtml('customerCompare.lostCustomersTitle','🚨 العملاء المفقودون')} ${customerCompareExportBtn('lostSimple')}</h4><p>${smartReportHtml('customerCompare.lostDescription','العملاء الذين اشتروا في سنة الأساس ولم يظهر لهم أي تعامل في سنة المقارنة طبقًا للفلاتر الحالية.')}</p>${customerCompareLostSimpleTableHtml}${customerCompareMore('customerCompareLostLimit',customerCompareLostRows,'units.lostCustomer')}</div>
        <div class="smart-panel customer-yoy-lost-panel">
          ...
          <div class="customer-yoy-kpis executive">
            <div class="new-cust-kpi"><span>${smartReportHtml('customerCompare.lostCustomerCount','عدد العملاء المفقودين')}</span><b>${fmt0(customerCompareLostRows.length)}</b><small>من عملاء ${customerCompareBaseYear}</small></div>
            <div class="new-cust-kpi"><span>${smartReportHtml('customerCompare.lostSalesValue','قيمة المبيعات المفقودة')}</span><b>${money(customerCompareLostValueTotal)}</b><small>${customerCompareValueLabel()}</small></div>
            <div class="new-cust-kpi"><span>${smartReportHtml('customerCompare.lostInvoiceCount','عدد الفواتير المفقودة')}</span><b>${fmt0(customerCompareLostInvoicesTotal)}</b><small>فواتير سنة الأساس</small></div>
            <div class="new-cust-kpi"><span>${smartReportHtml('customerCompare.lossRate','نسبة الفقد')}</span><b class="metric-down">${customerCompareLostPct.toFixed(1)}%</b><small>من إجمالي عملاء سنة الأساس</small></div>
          </div>
          ${customerCompareLostTableHtml}${customerCompareMore('customerCompareLostLimit',customerCompareLostRows,'units.lostCustomer')}
        </div>
        ...
        <div class="smart-panel"><h4>${smartReportHtml('customerCompare.allCustomersDetailed','📋 كل العملاء - مقارنة تفصيلية')} ${customerCompareExportBtn('all')}</h4>${customerCompareAllDetailedTableHtml}${customerCompareMore('customerCompareTableLimit',customerCompareMainRows,'عميل')}</div>
  ...
  const inactiveBuckets=[
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('period.days0to30','0-30 يوم'),min:0,max:30},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('period.days31to60','31-60 يوم'),min:31,max:60},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('period.days61to90','61-90 يوم'),min:61,max:90},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('period.days91to120','91-120 يوم'),min:91,max:120},
    {label:window.PETATOE_LOCALIZATION_CENTER.smart('period.days120Plus','120+ يوم'),min:121,max:99999}
  ...
  window.__petatoeContractCandidateDetails=allContractCandidates.map(x=>({
    ...
    lastDate:x.lastDate?smartFormatDate(x.lastDate):'—', days:x.daysSince||0, score:x.contractScore||0,
  ...
  const contractCandidateRows=displayedContractCandidates.map((x,i)=>{
    ...
    return `<tr>
      ...
      <td>${x.lastDate?smartFormatDate(x.lastDate):'—'}</td>
  ...
  const smartRecommendations=[
    ...
    {cat:'services',tab:'services',prio:'عالي',accent:'var(--purple)',ico:'🐾',title:'تحويل الخدمة الأعلى إلى Bundle',desc:`${htmlSafe(smartServiceDisplay(topService[0]))} تقود الإيراد بمتوسط ${money(recTopServiceAvg)} للعملية. اربطها بخدمة مكملة لرفع الفاتورة.`,impact:money(recTopServiceOps*recTopServiceAvg*0.12),confidence:'90%',report:'تحليل الخدمات',detail:['كوّن باقة من الخدمة الأعلى + خدمة مكملة.', 'اختبر سعرين مختلفين لمدة أسبوع.', 'راقب متوسط الفاتورة قبل وبعد الباقة.']},
    ...
    {cat:'management',tab:'customers',prio:'متوسط',accent:'var(--blue)',ico:'🏆',title:'نموذج العميل المثالي',desc:`أفضل عميل هو ${htmlSafe(topClient[0])} وأفضل خدمة هي ${htmlSafe(smartServiceDisplay(topService[0]))}. استخدم النمط لفهم العميل المثالي.`,impact:money((topClient[1]||0)*0.10),confidence:'80%',report:'Customer 360',detail:['افتح Customer 360 لأفضل العملاء.', 'استخرج الخدمات المشتركة بينهم.', 'حوّل النمط إلى عرض موجه لعملاء مشابهين.']}
  ];
  ...
  const ceoOpportunity=`باقة ${htmlSafe(smartServiceDisplay(topService[0]))} + خدمة مكملة قد ترفع الإيراد المتوقع ${money(recTopServiceOps*recTopServiceAvg*0.12)}.`;
  ...
  smartSafeHTML('smartReportsArea',`
    <div class="smart-actions-row" id="smartTabs"><button class="smart-pill active" data-smart-tab="overview" data-smart-action="smart-tab" data-tab="overview">${smartReportHtml('tabs.overview','ملخص الأداء')}</button><button class="smart-pill" data-smart-tab="sales" data-smart-action="smart-tab" data-tab="sales">${smartReportHtml('tabs.sales','تحليل المبيعات')}</button><button class="smart-pill" data-smart-tab="vehicles" data-smart-action="smart-tab" data-tab="vehicles">${smartReportHtml('tabs.vehicles','تحليل السيارات')}</button><button class="smart-pill" data-smart-tab="customers" data-smart-action="smart-tab" data-tab="customers">${smartReportHtml('tabs.customers','تحليل العملاء')}</button><button class="smart-pill" data-smart-tab="services" data-smart-action="smart-tab" data-tab="services">${smartReportHtml('tabs.services','تحليل الخدمات')}</button><button class="smart-pill" data-smart-tab="advanced" data-smart-action="smart-tab" data-tab="advanced">${smartReportHtml('tabs.advanced','مركز التقارير المتقدمة')}</button><button class="smart-pill smart-pill-merged-ai" data-smart-tab="forecast" data-smart-action="smart-tab" data-tab="forecast">${smartReportHtml('tabs.forecast','التوقعات وذكاء الأعمال')}</button><button class="smart-pill" data-smart-tab="salesInvoices" data-smart-action="smart-tab" data-pet-lazy-group="sales">${smartReportHtml('tabs.salesInvoices','إدارة فواتير المبيعات')}</button><button class="smart-pill" data-smart-tab="recommendations" data-smart-action="smart-tab" data-tab="recommendations">${smartReportHtml('tabs.recommendations','التوصيات')}</button></div>
    <div class="smart-tab-section active" data-smart-section="overview">${overviewCardsYearFilter}<div class="smart-exec-grid">
      <div class="smart-exec-kpi" style="--accent:var(--purple)">
        <button class="kpi-info-btn" type="button" aria-label="${smartReportHtml('overview.details.totalSales','تفاصيل إجمالي المبيعات')}">💰</button>
        <div class="kpi-tooltip"><div class="tip-head"><span class="tip-ico">💰</span><span>${smartReportHtml('overview.totalSales','إجمالي المبيعات')}</span></div><div class="tip-desc">${smartReportHtml('overview.desc.totalSales','يعرض إجمالي قيمة المبيعات للفترة أو السنة المحددة داخل التقارير الذكية.')}</div><div class="tip-grid"><div class="tip-row"><span>${smartReportHtml('overview.period','الفترة')}</span><b>${overviewCardsYearLabel}</b></div><div class="tip-row"><span>${smartReportHtml('overview.invoiceCount','عدد الفواتير')}</span><b>${fmt0(cardInvoices.length)}</b></div><div class="tip-row"><span>${smartReportHtml('overview.operationsCount','عدد العمليات')}</span><b>${fmt0(cardCount)}</b></div><div class="tip-row"><span>${smartReportHtml('overview.averageInvoice','متوسط الفاتورة')}</span><b>${money(cardAvg)}</b></div></div></div>
        <span>${smartReportHtml('overview.totalSales','إجمالي المبيعات')}</span><b>${money(cardTotal)}</b><small>${overviewCardsYearLabel}</small>
      </div>
      <div class="smart-exec-kpi" style="--accent:var(--cyan)">
        <button class="kpi-info-btn" type="button" aria-label="${smartReportHtml('overview.details.invoiceCount','تفاصيل عدد الفواتير')}">🧾</button>
        <div class="kpi-tooltip"><div class="tip-head"><span class="tip-ico">🧾</span><span>${smartReportHtml('overview.invoiceCount','عدد الفواتير')}</span></div><div class="tip-desc">${smartReportHtml('overview.desc.invoiceCount','يوضح عدد الفواتير الفريدة مقارنة بعدد العمليات الفعلية المسجلة في البيانات.')}</div><div class="tip-grid"><div class="tip-row"><span>${smartReportHtml('overview.invoiceCount','عدد الفواتير')}</span><b>${fmt0(cardInvoices.length)}</b></div><div class="tip-row"><span>${smartReportHtml('overview.operationsCount','عدد العمليات')}</span><b>${fmt0(cardCount)}</b></div><div class="tip-row"><span>${smartReportHtml('overview.dataRange','نطاق البيانات')}</span><b>${cardDateRange.first?smartFormatDate(cardDateRange.first):"—"} - ${cardDateRange.last?smartFormatDate(cardDateRange.last):"—"}</b></div></div></div>
        <span>${smartReportHtml('overview.invoiceCount','عدد الفواتير')}</span><b>${fmt0(cardInvoices.length)}</b><small>${fmt0(cardCount)} ${smartReportHtml('overview.operationUnit','عملية')}</small>
      </div>
      <div class="smart-exec-kpi" style="--accent:var(--green)">
        <button class="kpi-info-btn" type="button" aria-label="${smartReportHtml('overview.details.customerRetention','تفاصيل معدل الاحتفاظ بالعملاء')}">👥</button>
        <div class="kpi-tooltip"><div class="tip-head"><span class="tip-ico">👥</span><span>${smartReportHtml('overview.customerRetention','معدل الاحتفاظ بالعملاء')}</span></div><div class="tip-desc">${smartReportHtml('overview.desc.customerRetention','يقيس نسبة العملاء المتكررين من إجمالي العملاء خلال الفترة المختارة.')}</div><div class="tip-grid"><div class="tip-row"><span>${smartReportHtml('overview.repeatCustomers','عملاء متكررون')}</span><b>${fmt0(cardRepeatClients)}</b></div><div class="tip-row"><span>${smartReportHtml('overview.oneTimeCustomers','عملاء مرة واحدة')}</span><b>${fmt0(cardOneTimeClients)}</b></div><div class="tip-row"><span>${smartReportHtml('overview.totalCustomers','إجمالي العملاء')}</span><b>${fmt0(cardClients.length)}</b></div></div></div>
        <span>${smartReportHtml('overview.customerRetention','معدل الاحتفاظ بالعملاء')}</span><b>${cardClientRetention.toFixed(1)}%</b><small>${fmt0(cardRepeatClients)} ${smartReportHtml('overview.repeatCustomerUnit','عميل متكرر')}</small>
      </div>
      <div class="smart-exec-kpi" style="--accent:var(--orange)">
        <button class="kpi-info-btn" type="button" aria-label="${smartReportHtml('overview.details.averageInvoice','تفاصيل متوسط الفاتورة')}">📋</button>
        <div class="kpi-tooltip"><div class="tip-head"><span class="tip-ico">📋</span><span>${smartReportHtml('overview.averageInvoice','متوسط الفاتورة')}</span></div><div class="tip-desc">${smartReportHtml('overview.desc.averageInvoice','يعرض متوسط قيمة الفاتورة مع متوسط الإيراد اليومي حسب البيانات الحالية.')}</div><div class="tip-grid"><div class="tip-row"><span>${smartReportHtml('overview.averageInvoice','متوسط الفاتورة')}</span><b>${money(cardAvg)}</b></div><div class="tip-row"><span>${smartReportHtml('overview.dailyAverage','متوسط يومي')}</span><b>${money(cardDailyAvg)}</b></div><div class="tip-row"><span>${smartReportHtml('overview.operatingDays','أيام التشغيل')}</span><b>${fmt0(cardDaysCount)}</b></div></div></div>
        <span>${smartReportHtml('overview.averageInvoice','متوسط الفاتورة')}</span><b>${money(cardAvg)}</b><small>${smartReportHtml('overview.dailyAverage','متوسط يومي')} ${money(cardDailyAvg)}</small>
      </div>
      <div class="smart-exec-kpi" style="--accent:var(--blue)">
        <button class="kpi-info-btn" type="button" aria-label="${smartReportHtml('overview.details.topVehicle','تفاصيل أكفأ سيارة')}">🚐</button>
        <div class="kpi-tooltip"><div class="tip-head"><span class="tip-ico">🚐</span><span>${smartReportHtml('overview.topVehicle','أكفأ سيارة')}</span></div><div class="tip-desc">${smartReportHtml('overview.desc.topVehicle','ترتيب السيارة حسب متوسط الإيراد لكل عملية خلال الفترة المختارة.')}</div><div class="tip-grid"><div class="tip-row"><span>${smartReportHtml('overview.vehicle','السيارة')}</span><b>${cardBestEffVan[0]}</b></div><div class="tip-row"><span>${smartReportHtml('overview.revenue','الإيراد')}</span><b>${money(cardBestEffVan[1]||0)}</b></div><div class="tip-row"><span>${smartReportHtml('overview.operations','العمليات')}</span><b>${fmt0(cardBestEffVan[2]||0)}</b></div><div class="tip-row"><span>${smartReportHtml('overview.averageTransaction','متوسط العملية')}</span><b>${money(cardBestEffVan[3]||0)}</b></div></div></div>
        <span>${smartReportHtml('overview.topVehicle','أكفأ سيارة')}</span><b>${cardBestEffVan[0]}</b><small>${money(cardBestEffVan[3])} / ${smartReportHtml('overview.operationUnit','عملية')}</small>
      </div>
      <div class="smart-exec-kpi" style="--accent:var(--pink)">
        <button class="kpi-info-btn" type="button" aria-label="${smartReportHtml('overview.details.bestService','تفاصيل أفضل خدمة')}">⭐</button>
        <div class="kpi-tooltip"><div class="tip-head"><span class="tip-ico">⭐</span><span>${smartReportHtml('overview.bestService','أفضل خدمة')}</span></div><div class="tip-desc">${smartReportHtml('overview.desc.bestService','الخدمة الأعلى تحقيقًا للمبيعات في الفترة المختارة من واقع البيانات المرفوعة.')}</div><div class="tip-grid"><div class="tip-row"><span>${smartReportHtml('overview.service','الخدمة')}</span><b>${htmlSafe(smartServiceDisplay(cardTopService[0]))}</b></div><div class="tip-row"><span>${smartReportHtml('overview.totalSales','إجمالي المبيعات')}</span><b>${money(cardTopService[1])}</b></div><div class="tip-row"><span>${smartReportHtml('overview.operationsCount','عدد العمليات')}</span><b>${fmt0(overviewCardsData.filter(r=>(r.item||'غير محدد')===cardTopService[0]).length)}</b></div><div class="tip-row"><span>${smartReportHtml('overview.topFiveServicesShare','مساهمة أعلى 5 خدمات')}</span><b>${cardServiceTop5Share.toFixed(1)}%</b></div></div></div>
        <span>${smartReportHtml('overview.bestService','أفضل خدمة')}</span><b>${htmlSafe(smartServiceDisplay(cardTopService[0]))}</b><small>${money(cardTopService[1])}</small>
      </div>
    </div>
    ...
    <div class="smart-dash-grid smart-overview-stacked">
      ...
      <div class="smart-panel" id="smartVehicleEfficiencyPanel"><div class="smart-panel-head-with-filters"><div><h3>🚐 ${smartReportHtml('vehicleEfficiency.title','تحليل كفاءة السيارات')}</h3></div>${smartVehicleEfficiencyFilterHtml()}</div><div class="smart-table-clean"><table><thead><tr><th>${smartReportHtml('overview.vehicle','السيارة')}</th><th>${smartReportHtml('vehicleEfficiency.operationsCount','عدد العمليات')}</th><th>${smartReportHtml('overview.revenue','الإيراد')}</th><th>${smartReportHtml('overview.averageTransaction','متوسط العملية')}</th><th>${smartReportHtml('vehicleEfficiency.contribution','المساهمة')}</th></tr></thead><tbody id="smartVehicleEfficiencyBody">${smartVehicleEfficiencyRowsHtml()}</tbody></table></div></div>
    </div>
    ...
    <div class="smart-tab-section" data-smart-section="customers">
      ...
      <div class="smart-panel">
        <div class="new-cust-report-head"><h3>${smartReportHtml('newCustomers.monthTitle','🆕 العملاء الجدد خلال الشهر')}</h3><div class="new-cust-year-controls">${newCustYearButtons}</div></div>
        <p>${smartReportHtml('newCustomers.monthDescription','يعرض العملاء الذين ظهرت أول معاملة لهم فعليًا في الشهر المختار، مقارنة بكل البيانات التاريخية المتاحة. إذا كان الشهر فارغًا فهذا يعني أن العملاء الموجودين سبق ظهورهم قبل هذا الشهر.')}</p>
        ...
        <div class="new-cust-kpis">
          ...
          <div class="new-cust-kpi"><span>${smartReportHtml('newCustomers.averageNewCustomer','متوسط العميل الجديد')}</span><b>${money(newCustomersAvg)}</b><small>${smartReportHtml('newCustomers.totalDividedByCustomers','إجمالي / عدد العملاء')}</small></div>
        ...
        <div class="new-cust-logic-note"><b>${smartReportHtml('newCustomers.calculationTitle','طريقة احتساب العملاء الجدد:')}</b><br>${smartReportHtml('newCustomers.calculationDescription','يتم احتساب العميل الجديد عند تاريخ أول معاملة للعميل في كل قاعدة البيانات، ثم يُعرض فقط إذا كانت أول معاملة داخل الشهر والسنة المختارة. أي أن العميل لا يُحتسب كعميل جديد مرة أخرى إذا كان له أي معاملة أقدم.')}</div>
      </div>

      <div class="new-cust-grid">
        <div class="smart-panel"><h3>${smartReportHtml('newCustomers.weeklyTitle','📈 توزيع العملاء الجدد أسبوعيًا')}</h3><p>${smartReportHtml('newCustomers.weeklyDescription','عدد العملاء الجدد حسب أسبوع أول معاملة داخل الشهر.')}</p><div class="new-cust-chart sm"><canvas id="newCustomersWeeklyChart"></canvas></div></div>
      ...
      <div class="smart-panel"><h3>📋 ${smartReportHtml('customers.newDetailsTitle','تفاصيل العملاء الجدد والمعاملات')}</h3><p>${smartReportHtml('customers.newDetailsDescription','العميل، تاريخ أول معاملة، رقم الفاتورة، الخدمة، طريقة الدفع، عدد العمليات، إجمالي المبيعات، وآخر زيارة.')}</p><div class="smart-table-clean new-cust-table"><table><thead><tr><th>#</th><th>${smartReportHtml('table.customer','العميل')}</th><th>${smartReportHtml('table.firstTransaction','أول معاملة')}</th><th>${smartReportHtml('table.invoiceNumber','رقم الفاتورة')}</th><th>${smartReportHtml('table.topService','أهم خدمة')}</th><th>${smartReportHtml('table.topPaymentMethod','أهم طريقة دفع')}</th><th>${smartReportHtml('vehicleEfficiency.operationsCount','عدد العمليات')}</th><th>${smartReportHtml('overview.totalSales','إجمالي المبيعات')}</th><th>${smartReportHtml('overview.averageTransaction','متوسط العملية')}</th><th>${smartReportHtml('table.lastTransaction','آخر معاملة')}</th><th>${smartReportHtml('table.daysSinceLastVisit','أيام منذ آخر زيارة')}</th><th>${smartReportHtml('customers.classification','التصنيف')}</th></tr></thead><tbody>${newCustomerTableRows}</tbody></table></div>${newCustomerMoreButton}</div>

      <div class="smart-panel"><h3>🚨 ${smartReportHtml('customers.newInactiveTitle','العملاء الجدد غير النشطين')}</h3><p>${smartReportHtml('customers.newInactiveDescription','عملاء قاموا بأول معاملة ولم يظهر لهم نشاط لاحق واضح حتى تاريخ آخر بيانات متاحة.')}</p><div class="smart-table-clean"><table><thead><tr><th>#</th><th>${smartReportHtml('table.customer','العميل')}</th><th>${smartReportHtml('table.firstTransaction','أول معاملة')}</th><th>${smartReportHtml('table.invoiceNumber','رقم الفاتورة')}</th><th>${smartReportHtml('table.transactionValue','قيمة المعاملة')}</th><th>${smartReportHtml('table.daysSinceLastVisit','أيام منذ آخر زيارة')}</th><th>${smartReportHtml('table.service','الخدمة')}</th></tr></thead><tbody>${newCustomerInactiveRows}</tbody></table></div></div>

      <div class="smart-panel"><h3>🏆 ${smartReportHtml('customers.valueAnalysisTitle','تحليل قيمة العملاء')}</h3><p>${smartReportHtml('customers.valueAnalysisDescription','أعلى العملاء حسب صافي الإنفاق بعد استبعاد فواتير البيع الملغاة بمرتجع كامل، مع تفاصيل الزيارات وتصنيف العميل.')}</p><div class="smart-table-clean"><table><thead><tr><th>${smartReportHtml('table.customer','العميل')}</th><th>${smartReportHtml('table.totalSpending','إجمالي الإنفاق')}</th><th>${smartReportHtml('table.visitCount','عدد الزيارات')}</th><th>${smartReportHtml('overview.averageInvoice','متوسط الفاتورة')}</th><th>${smartReportHtml('table.lastVisitDate','تاريخ آخر زيارة')}</th><th>${smartReportHtml('table.lastInvoiceValue','قيمة آخر فاتورة')}</th><th>${smartReportHtml('table.visitMonths','شهور الزيارة')}</th><th>${smartReportHtml('table.daysSinceLastVisit','أيام منذ آخر زيارة')}</th><th>${smartReportHtml('customers.customerClassification','تصنيف العميل')}</th></tr></thead><tbody>${customerInsightRows}</tbody></table></div>${customerInsightMoreButton}</div>
      ...
      <div class="customer-analysis-pane ${(['overview','contracts','compare','ai'].includes(window.customerAnalysisSubTab)?window.customerAnalysisSubTab:'overview')==='ai'?'active':''}" data-customer-analysis-pane="ai">
        ...
        <div class="smart-panel">${inactiveActivityPanelHead(window.PETATOE_LOCALIZATION_CENTER.smart('customers.absenceDistributionTitle','📊 توزيع العملاء حسب مدة الغياب'),'aging')}<p>${smartReportHtml('customers.absenceDistributionDescription','يوضح أين تتركز مخاطر فقد العملاء بناءً على تاريخ آخر زيارة صافية.')}</p><div class="inactive-cust-chart"><canvas id="inactiveAgingChart"></canvas></div></div>
    ...
    <div class="smart-tab-section" data-smart-section="salesInvoices"><div id="salesInvoiceReportArea"></div></div>

    <div class="smart-tab-section" data-smart-section="recommendations">
      <div class="smart-panel">
        <h3>${smartReportHtml('recommendations.advancedTitle','🧠 التوصيات الذكية المتقدمة')}</h3>
        <p>${smartReportHtml('recommendations.advisorDescription','Business Advisor تفاعلي مبني على بيانات المبيعات والعملاء والسيارات والخدمات داخل PETATOE.')}</p>
        ${ceoBriefHtml}
        <div class="smart-rec-pro-toolbar">
          <div class="smart-rec-filter-row">
            <button class="smart-rec-filter active" data-rec-filter="revenue" data-smart-action="recommendation-filter" data-filter="revenue">${smartReportHtml('recommendations.categories.revenue','الإيرادات')}</button>
            <button class="smart-rec-filter" data-rec-filter="customers" data-smart-action="recommendation-filter" data-filter="customers">${smartReportHtml('recommendations.categories.customers','العملاء')}</button>
            <button class="smart-rec-filter" data-rec-filter="vehicles" data-smart-action="recommendation-filter" data-filter="vehicles">${smartReportHtml('recommendations.categories.vehicles','السيارات')}</button>
            <button class="smart-rec-filter" data-rec-filter="services" data-smart-action="recommendation-filter" data-filter="services">${smartReportHtml('recommendations.categories.services','الخدمات')}</button>
            <button class="smart-rec-filter" data-rec-filter="pets" data-smart-action="recommendation-filter" data-filter="pets">${smartReportHtml('recommendations.categories.pets','الحيوانات الأليفة')}</button>
            <button class="smart-rec-filter" data-rec-filter="profit" data-smart-action="recommendation-filter" data-filter="profit">${smartReportHtml('recommendations.categories.profit','الربحية')}</button>
            <button class="smart-rec-filter" data-rec-filter="capacity" data-smart-action="recommendation-filter" data-filter="capacity">${smartReportHtml('recommendations.categories.capacity','الطاقة التشغيلية')}</button>
            <button class="smart-rec-filter" data-rec-filter="management" data-smart-action="recommendation-filter" data-filter="management">${smartReportHtml('recommendations.categories.management','إدارية')}</button>
            <button class="smart-rec-filter" data-rec-filter="forecast" data-smart-action="recommendation-filter" data-filter="forecast">${smartReportHtml('recommendations.categories.forecast','التوقعات')}</button>
          </div>
  ...
  const salesIntelMonthlyConfig={gross:{label:window.PETATOE_LOCALIZATION_CENTER.smart('metrics.gross','شامل الضريبة'),data:salesIntelMonthlyGross,color:css('--purple'),bg:'rgba(139,92,246,.16)'},net:{label:window.PETATOE_LOCALIZATION_CENTER.smart('metrics.net','قبل الضريبة'),data:salesIntelMonthlyNet,color:css('--blue'),bg:'rgba(59,130,246,.14)'},tax:{label:window.PETATOE_LOCALIZATION_CENTER.smart('metrics.vat','الضريبة'),data:salesIntelMonthlyTax,color:css('--green'),bg:'rgba(34,197,94,.14)'}}[salesIntelMonthlyMode]||{label:window.PETATOE_LOCALIZATION_CENTER.smart('metrics.gross','شامل الضريبة'),data:salesIntelMonthlyGross,color:css('--purple'),bg:'rgba(139,92,246,.16)'};
  ...
  chart('salesIntelMonthCompare',{
      ...
      afterDatasetsDraw(chart){
        ...
        rows.forEach(function(row,i){
          ...
          if(c){
            ...
            ctx.fillText(window.PETATOE_LOCALIZATION_CENTER.smart('compare.current','الحالي'),c.x,yBase);
          ...
          if(p){
            ...
            ctx.font='900 10px Cairo';
            ctx.fillText(window.PETATOE_LOCALIZATION_CENTER.smart('compare.previous','السابق'),p.x,yBase);
            ctx.font='800 9px Cairo';
    ...
    data:{labels:salesComparisonRows.map(r=>r.label),datasets:[
      {label:window.PETATOE_LOCALIZATION_CENTER.smart('compare.current','الحالي'),data:salesComparisonRows.map(r=>r.current),backgroundColor:css('--comparison-current')||css('--cyan'),borderRadius:10},
      {label:window.PETATOE_LOCALIZATION_CENTER.smart('compare.previous','السابق'),data:salesComparisonRows.map(r=>r.previous),backgroundColor:css('--comparison-previous')||css('--gold-bright'),borderRadius:10},
      {label:window.PETATOE_LOCALIZATION_CENTER.smart('compare.difference','الفرق'),data:salesComparisonRows.map(r=>r.diff),backgroundColor:function(ctx){return (ctx.raw||0)<0?(css('--comparison-diff-negative')||'#FF5A7A'):(css('--comparison-diff-positive')||'#A855F7')},borderRadius:10}
    ]},
    options:{
      ...baseOpts(),
  ...
  chart('salesIntelNewReturning',{type:'doughnut',data:{labels:[window.PETATOE_LOCALIZATION_CENTER.smart('customers.oneTime','عملاء مرة واحدة'),window.PETATOE_LOCALIZATION_CENTER.smart('customers.returning','عملاء متكررون')],datasets:[{label:window.PETATOE_LOCALIZATION_CENTER.smart('customers.classification','تصنيف العملاء'),data:[salesNewReturning.new,salesNewReturning.returning],backgroundColor:[css('--blue'),css('--gold')||'#FFD54A'],borderWidth:0}]},options:{...baseOpts('bottom'),cutout:'58%',plugins:{...baseOpts('bottom').plugins,tooltip:{enabled:false},legend:{position:'bottom',labels:{color:'#fff',font:{family:'Cairo',weight:'900'},generateLabels:function(chart){return [{text:window.PETATOE_LOCALIZATION_CENTER.smart('customers.oneTime','عملاء مرة واحدة'),fillStyle:css('--blue')||'#18E7F9',strokeStyle:css('--blue')||'#18E7F9',fontColor:'#fff',hidden:false,index:0},{text:window.PETATOE_LOCALIZATION_CENTER.smart('customers.returning','عملاء متكررون'),fillStyle:css('--gold')||'#FFD54A',strokeStyle:css('--gold')||'#FFD54A',fontColor:'#fff',hidden:false,index:1}];}}},petatoeLabels:{enabled:true,money:false,color:'#fff',font:'900 11px Cairo'}}}});
  ...
  if(preservedSmartTab==='customers'){
    chart('newCustomersWeeklyChart',{type:'bar',data:{labels:newCustWeekRows.map(x=>x.label),datasets:[{label:window.PETATOE_LOCALIZATION_CENTER.smart('customers.newCustomers','عملاء جدد'),data:newCustWeekRows.map(x=>x.count),backgroundColor:css('--purple'),borderRadius:10}]},options:{...baseOpts(),layout:{padding:{top:24}},plugins:{...baseOpts().plugins,petatoeLabels:{enabled:true,money:false,font:'900 11px Cairo'}}}});
    chart('newCustomersTopValueChart',{type:'bar',data:{labels:newCustomerRows.slice(0,10).map(x=>x.name),datasets:[{label:window.PETATOE_LOCALIZATION_CENTER.smart('metrics.sales','المبيعات'),data:newCustomerRows.slice(0,10).map(x=>x.totalValue),backgroundColor:css('--blue'),borderRadius:10}]},options:{...baseOpts(),indexAxis:'y',layout:{padding:{left:12,right:12}},plugins:{...baseOpts().plugins,petatoeLabels:{enabled:true,fullMoney:true,font:'900 10px Cairo'}}}});
    chart('newCustomersTrendChart',{type:'line',data:{labels:newCustTrendRows.map(x=>x.label),datasets:[{label:window.PETATOE_LOCALIZATION_CENTER.smart('customers.newCustomers','عملاء جدد'),data:newCustTrendRows.map(x=>x.count),borderColor:css('--green'),backgroundColor:'rgba(34,197,94,.16)',tension:.35,pointRadius:4,fill:true}]},options:{...baseOpts(),layout:{padding:{top:24}},plugins:{...baseOpts().plugins,petatoeLabels:{enabled:true,money:false,font:'900 11px Cairo'}}}});
    chart('inactiveAgingChart',{type:'bar',data:{labels:inactiveBuckets.map(x=>x.label),datasets:[{label:window.PETATOE_LOCALIZATION_CENTER.smart('customers.customerCount','عدد العملاء'),data:inactiveBuckets.map(x=>x.count),backgroundColor:css('--orange'),borderRadius:10}]},options:{...baseOpts(),layout:{padding:{top:26}},plugins:{...baseOpts().plugins,petatoeLabels:{enabled:true,money:false,font:'900 11px Cairo'}}}});
    chart('inactiveLostTrendChart',{type:'line',data:{labels:inactiveTrendRows.map(x=>x.label),datasets:[{label:window.PETATOE_LOCALIZATION_CENTER.smart('customers.becameInactive','عملاء أصبحوا غير نشطين'),data:inactiveTrendRows.map(x=>x.count),borderColor:css('--red'),backgroundColor:'rgba(239,68,68,.16)',tension:.35,pointRadius:4,fill:true,clip:false}]},options:{...baseOpts(),layout:{padding:{top:28,right:54,left:54,bottom:34}},plugins:{...baseOpts().plugins,petatoeLabels:{enabled:true,money:false,font:'900 11px Cairo',offset:12}},scales:{x:{offset:true,ticks:{color:css('--text'),font:{family:'Cairo',weight:'900'},autoSkip:false,maxRotation:0,minRotation:0,padding:14},grid:{color:'rgba(148,163,184,.12)'}},y:{ticks:{color:css('--muted'),font:{family:'Cairo',weight:'800'}},grid:{color:'rgba(148,163,184,.13)'}}}}});
