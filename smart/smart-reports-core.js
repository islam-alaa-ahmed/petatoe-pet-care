...
function renderSmartReports(){
  ...
  const salesAlerts=[];
  const lastMonthLabel=salesLastMonth.m&&salesLastMonth.m!=='-'?MAR[salesLastMonth.m]:'آخر شهر نشط';
  const prevMonthLabel=salesPrevMonth.m&&salesPrevMonth.m!=='-'?MAR[salesPrevMonth.m]:'الشهر السابق';
  const monthTrendDetail=`<b>تفاصيل تغير المبيعات الشهرية</b><div class="tip-grid">${alertDetailBox('الشهر الحالي',`${lastMonthLabel}`)}${alertDetailBox('مبيعات الشهر الحالي',money(salesLastMonth.sales||0))}${alertDetailBox('الشهر السابق',`${prevMonthLabel}`)}${alertDetailBox('مبيعات الشهر السابق',money(salesPrevMonth.sales||0))}${alertDetailBox('الفرق',`${salesMonthDiff>=0?'+':'-'} ${money(Math.abs(salesMonthDiff))}`)}${alertDetailBox('نسبة التغير',`${salesMonthDiffPct.toFixed(1)}%`)}</div><span class="${salesMonthDiff>=0?'ok':'badline'}">${salesMonthDiff>=0?'✓ يوجد تحسن عن الشهر السابق.':'⚠ يوجد انخفاض عن الشهر السابق ويحتاج مراجعة.'}</span><span>التوصية: راجع العملاء غير النشطين، الخدمات الأقل حركة، وأداء السيارات خلال الشهر الحالي مقارنة بالشهر السابق.</span>`;
  salesAlerts.push({type:salesMonthDiff>=0?'good':'bad',text:`${salesMonthDiff>=0?'ارتفاع':'انخفاض'} آخر شهر نشط عن الشهر السابق بقيمة ${money(Math.abs(salesMonthDiff))} (${salesMonthDiffPct.toFixed(1)}%).`,detail:monthTrendDetail});
  if(bestEffVan[0]&&bestEffVan[0]!=='-'){
    const vanRank=vanAvgRev.findIndex(v=>v[0]===bestEffVan[0])+1;
    const vanDetail=`<b>تفاصيل كفاءة السيارة</b><div class="tip-grid">${alertDetailBox('السيارة',htmlSafe(bestEffVan[0]))}${alertDetailBox('الترتيب',`#${fmt0(vanRank||1)} من ${fmt0(vanAvgRev.length)}`)}${alertDetailBox('إجمالي المبيعات',money(bestEffVan[1]||0))}${alertDetailBox('عدد العمليات',fmt0(bestEffVan[2]||0))}${alertDetailBox('متوسط العملية',money(bestEffVan[3]||0))}${alertDetailBox('المساهمة',safeDiv(bestEffVan[1]||0,total).toLocaleString('en-US',{style:'percent',maximumFractionDigits:1}))}</div><span class="ok">✓ تم اختيار السيارة بناءً على أعلى متوسط إيراد لكل عملية.</span><span>التوصية: استخدم نمط تشغيل هذه السيارة كمرجع لباقي السيارات الأقل كفاءة.</span>`;
    salesAlerts.push({type:'good',text:`السيارة الأعلى كفاءة هي ${bestEffVan[0]} بمتوسط ${money(bestEffVan[3]||0)} لكل عملية.`,detail:vanDetail});
  }
  if(salesAtRiskRows.length){
    const riskRows=salesAtRiskRows.slice(0,8).map((r,i)=>`<tr><td>${i+1}</td><td>${htmlSafe(r.name)}</td><td>${r.last?fmtDateAr(r.last):'—'}</td><td>${fmt0(r.days)} يوم</td><td>${money(r.value)}</td></tr>`).join('');
    const riskDetail=`<b>تفاصيل العملاء الكبار غير النشطين</b><div class="tip-grid">${alertDetailBox('عدد العملاء',fmt0(salesAtRiskRows.length))}${alertDetailBox('شرط الإدراج','أكثر من 60 يوم')}${alertDetailBox('أعلى مدة غياب',fmt0(Math.max(...salesAtRiskRows.map(r=>r.days)))+' يوم')}${alertDetailBox('إجمالي إنفاقهم',money(salesAtRiskRows.reduce((s,r)=>s+(r.value||0),0)))}</div><span>أعلى العملاء المعرضين للفقد:</span><div class="sales-alert-risk-table-wrap"><table class="sales-alert-risk-table"><thead><tr><th>#</th><th>العميل</th><th>آخر زيارة</th><th>أيام الغياب</th><th>إجمالي الإنفاق</th></tr></thead><tbody>${riskRows}</tbody></table></div><span class="ok">✓ الأفضل مراجعة تقرير العملاء غير النشطين وفرص الاسترجاع وترتيب المتابعة حسب أعلى إنفاق وأطول غياب.</span>`;
    salesAlerts.push({type:'warn',text:`يوجد ${fmt0(salesAtRiskRows.length)} عميل من كبار العملاء لم يزوروا منذ أكثر من 60 يوم.`,detail:riskDetail});
  }
  if(topService[0]&&topService[0]!=='-'){
    ...
    const topServiceDetail=`<b>تفاصيل أعلى خدمة قيمة</b><div class="tip-grid">${alertDetailBox('الخدمة',htmlSafe(topService[0]))}${alertDetailBox('إجمالي المبيعات',money(topService[1]||0))}${alertDetailBox('عدد العمليات',fmt0(topServiceOps))}${alertDetailBox('متوسط العملية',money(topServiceAvg))}${alertDetailBox('مساهمة الخدمة',safeDiv(topService[1]||0,total).toLocaleString('en-US',{style:'percent',maximumFractionDigits:1}))}${alertDetailBox('مساهمة أعلى 5 خدمات',`${serviceTop5Share.toFixed(1)}%`)}</div><span class="ok">✓ الخدمة الأعلى قيمة تم تحديدها من إجمالي مبيعات الخدمة في الفترة المختارة.</span><span>التوصية: اعمل Bundle أو عرض مكمل لهذه الخدمة لرفع متوسط الفاتورة.</span>`;
  ...
  smartSafeHTML('smartReportsArea',`
    ...
    <div class="smart-dash-grid smart-overview-stacked">
      ...
      <div class="smart-panel" id="smartVehicleEfficiencyPanel"><div class="smart-panel-head-with-filters"><div><h3>🚐 ${smartReportHtml('vehicleEfficiency.title','تحليل كفاءة السيارات')}</h3></div>${smartVehicleEfficiencyFilterHtml()}</div><div class="smart-table-clean"><table><thead><tr><th>${smartReportHtml('vehicleEfficiency.vehicle','السيارة')}</th><th>${smartReportHtml('vehicleEfficiency.operationsCount','عدد العمليات')}</th><th>${smartReportHtml('vehicleEfficiency.revenue','الإيراد')}</th><th>${smartReportHtml('vehicleEfficiency.averageTransaction','متوسط العملية')}</th><th>${smartReportHtml('vehicleEfficiency.contribution','المساهمة')}</th></tr></thead><tbody id="smartVehicleEfficiencyBody">${smartVehicleEfficiencyRowsHtml()}</tbody></table></div></div>
    </div>
    ...
    <div class="sales-intel-section">
      ...
      <div class="sales-intel-grid" style="grid-template-columns:1fr">
        <div class="sales-intel-panel"><h3>🔥 خريطة المبيعات Heatmap</h3>${heatHtml}</div>
      </div>
      <div class="sales-intel-grid four">
        <div class="sales-intel-panel"><h3>👥 عملاء مرة واحدة مقابل المتكررين</h3><div class="sales-intel-chart sm"><canvas id="salesIntelNewReturning"></canvas></div><div id="newReturningSummary" class="liquid-summary-row"></div></div>
        <div class="sales-intel-panel"><div class="smart-report-head-actions"><div><h3>🚨 العملاء المعرضين للفقد</h3><p>عملاء لم يزوروا منذ أكثر من 60 يوم.</p></div><div class="smart-report-head-buttons">${salesAtRiskRows.length?`<button type="button" class="btn btn-green" data-smart-action="export-at-risk-clients">⬇️ تصدير Excel</button>`:''}</div></div><div class="sales-intel-table"><table><thead><tr><th>العميل</th><th>آخر زيارة</th><th>الأيام</th><th>الإنفاق</th></tr></thead><tbody>${salesAtRiskRows.length?salesAtRiskRows.slice(0,window.smartAtRiskLimit||10).map(r=>`<tr><td>${r.name}</td><td>${r.last?fmtDateAr(r.last):'—'}</td><td>${fmt0(r.days)}</td><td>${money(r.value)}</td></tr>`).join(''):'<tr><td colspan="4">لا يوجد عملاء معرضون للفقد حسب شرط 60 يوم.</td></tr>'}</tbody></table></div><div class="smart-table-actions">${salesAtRiskRows.length>(window.smartAtRiskLimit||10)?`<button type="button" class="btn btn-ghost" data-smart-action="at-risk-more">عرض المزيد (${Math.min(salesAtRiskRows.length,(window.smartAtRiskLimit||10)+10)} / ${salesAtRiskRows.length})</button>`:''}</div></div>
        <div class="sales-intel-panel sales-target-active-panel"><h3>🎯 مؤشر تحقيق الهدف الشهري</h3><button class="sales-target-side-toggle" data-smart-action="sales-target-toggle">✏️ تعديل / إضافة الهدف</button><div class="sales-target-control-box"><div class="sales-target-year-row">${salesTargetYearButtons}</div><div class="sales-target-month-row">${salesTargetMonthButtons}</div></div><div id="salesTargetEditorPanel" class="sales-target-editor-panel"><div class="sales-target-editor-head"><b>تعديل / إضافة الهدف</b><button data-smart-action="sales-target-toggle">×</button></div><div class="sales-target-input-row"><div><label>الهدف الشهري لشهر ${MAR[salesTargetMonthName]} ${salesTargetYear}</label><input id="salesTargetInput" class="sales-target-ready" type="number" min="0" step="0.01" value="${salesTarget}" placeholder="اكتب الهدف بالريال"></div><button class="sales-target-edit-btn" data-smart-action="sales-target-enable-edit">✏️ تعديل الهدف</button><button class="sales-target-save-btn" data-smart-action="sales-target-save" data-clear="false">💾 حفظ الهدف</button><button class="sales-target-clear-btn" data-smart-action="sales-target-save" data-clear="true">إرجاع الافتراضي</button></div></div><div class="sales-target-gauge-wrap"><div class="sales-target-gauge-card"><div class="sales-target-gauge-title">مؤشر تحقيق الهدف</div><div class="gauge-target-side">الهدف الشهري<b>${salesTarget?fmt0(salesTarget):'—'}</b>ريال سعودي</div><svg class="sales-target-gauge-svg" viewBox="0 0 320 205" aria-label="مؤشر تحقيق الهدف الشهري"><path class="gauge-track" d="M40 150 A120 120 0 0 1 280 150"/><path class="gauge-arc red" pathLength="100" stroke-dasharray="32 68" stroke-dashoffset="0" d="M40 150 A120 120 0 0 1 280 150"/><path class="gauge-arc orange" pathLength="100" stroke-dasharray="33 67" stroke-dashoffset="-32" d="M40 150 A120 120 0 0 1 280 150"/><path class="gauge-arc green" pathLength="100" stroke-dasharray="35 65" stroke-dashoffset="-65" d="M40 150 A120 120 0 0 1 280 150"/><line class="gauge-tick" x1="62" y1="133" x2="73" y2="137"/><line class="gauge-tick" x1="101" y1="72" x2="109" y2="82"/><line class="gauge-tick" x1="160" y1="48" x2="160" y2="61"/><line class="gauge-tick" x1="219" y1="72" x2="211" y2="82"/><line class="gauge-tick" x1="258" y1="133" x2="247" y2="137"/><line class="gauge-needle" x1="160" y1="150" x2="${salesTargetNeedleX}" y2="${salesTargetNeedleY}"/><circle class="gauge-needle-core" cx="160" cy="150" r="9"/></svg><div class="gauge-scale"><span>0%</span><span>100%</span></div><div class="gauge-actual">${salesTarget?fmt(salesTargetActual):'غير محدد'}</div><div class="gauge-currency">ريال سعودي</div><div class="gauge-status-line">${salesTarget?`${salesTargetPct.toFixed(1)}% من الهدف الشهري`:'لم يتم إدخال هدف لهذا الشهر'}</div></div></div><div class="sales-target-kpi-grid"><div class="sales-target-kpi"><span>مبيعات الشهر الفعلية</span><b>${money(salesTargetActual)}</b></div><div class="sales-target-kpi"><span>الهدف المحفوظ</span><b>${salesTarget?money(salesTarget):'—'}</b></div><div class="sales-target-kpi"><span>${salesTargetActual>=salesTarget&&salesTarget?'تجاوز الهدف':'المتبقي لتحقيق الهدف'}</span><b>${salesTarget?(salesTargetActual>=salesTarget?money(salesTargetOver):money(salesTargetRemaining)):'—'}</b></div><div class="sales-target-kpi"><span>عدد العمليات / العملاء</span><b>${fmt0(salesTargetTransactions)} / ${fmt0(salesTargetClients)}</b></div></div><div class="sales-target-month-note"><b>${salesTargetStatus}</b> — يتم حساب المؤشر من مبيعات ${MAR[salesTargetMonthName]} ${salesTargetYear} فقط، مع الاحتفاظ بتاريخ أهداف الشهور السابقة وتطبيق أي تعديل على الشهر المختار وما بعده فقط.</div></div>
        <div class="sales-intel-panel"><h3>🔮 توقعات المبيعات</h3><p>اعتمادًا على Run Rate للشهور الفعلية الحالية.</p><div class="sales-alert-list"><div class="sales-alert good">الشهر القادم: ${money(salesForecastNext)}</div><div class="sales-alert warn">الربع القادم: ${money(salesForecastQuarter)}</div><div class="sales-alert good">نهاية السنة: ${money(salesForecastYearEnd)}</div></div></div>
      </div>
      <div class="sales-intel-panel"><h3>🧠 التنبيهات الذكية</h3><p>ملخص تلقائي لأهم الملاحظات من بيانات المبيعات الحالية.</p><div class="sales-alert-list">${salesAlerts.map(a=>`<div class="sales-alert ${a.type}" tabindex="0"><span>${a.text}</span><span class="sales-alert-info">ⓘ</span><div class="sales-alert-tooltip">${a.detail||'<b>تفاصيل التنبيه</b><span>لا توجد تفاصيل إضافية.</span>'}</div></div>`).join('')}</div></div>
    </div>
