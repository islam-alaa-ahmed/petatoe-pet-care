...
function smartListJoin(values){
  var separator=smartReportT('format.listSeparator','، ');
...
function renderSmartReports(){
  ...
  if(topService[0]&&topService[0]!=='-'){
    ...
    salesAlerts.push({type:'good',text:smartReportT('alerts.topServiceText','الخدمة الأعلى قيمة هي {service} بإجمالي {total}.',{service:smartServiceDisplay(topService[0]),total:money(topService[1])}),detail:topServiceDetail});
  }
  ...
  const customerComparePeriodLabel=customerCompareUseFullYear
    ...
    : (customerCompareLatestDate
      ? smartReportT('period.ytdComparison','وضع المقارنة: حتى الفترة الحالية — من 01 يناير {targetYear} إلى {targetEnd} مقابل نفس الفترة من {baseYear} حتى {baseEnd} — آخر فاتورة مرفوعة: {invoice}',{targetYear:customerCompareTargetYear,targetEnd:smartFormatDate(customerCompareTargetEnd),baseYear:customerCompareBaseYear,baseEnd:smartFormatDate(customerCompareBaseEnd),invoice:htmlSafe(String(customerCompareLatestRecord.invoice||'—'))})
  ...
  const customerCompareAllDetailedTableHtml=(function(){
    ...
    return window.PETATOETables.render({
      ...
      columns:[
        ...
        {label:smartReportT('table.lastInteraction','آخر تعامل'), render:function(x){return x.lastDate?smartFormatDate(x.lastDate):'—';}},
  ...
  const customerCompareLostRiskMeta=function(v){
    if(v>=20000) return {label:smartReportT('risk.high','مرتفع'), cls:'bad', icon:'🔴'};
    if(v>=5000) return {label:smartReportT('risk.medium','متوسط'), cls:'warn', icon:'🟡'};
    return {label:smartReportT('risk.low','منخفض'), cls:'good', icon:'🟢'};
  };
  ...
  const customerCompareLostSimpleTableHtml=customerCompareRenderVirtualTable('customerCompareLostSimpleVirtualTable',customerCompareLostRows,customerCompareLostLimit,[
    ...
    {label:smartReportT('table.lastInvoice','آخر فاتورة'),render:(x)=>x.baseLast?smartFormatDate(x.baseLast):'—'},
  ...
  const customerCompareLostTableHtml=customerCompareRenderVirtualTable('customerCompareLostDetailedVirtualTable',customerCompareLostRows,customerCompareLostLimit,[
    ...
    {label:smartReportT('table.lastVisit','آخر زيارة'),render:(x)=>x.baseLast?smartFormatDate(x.baseLast):'—'},
