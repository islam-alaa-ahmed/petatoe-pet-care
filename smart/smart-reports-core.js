...
function renderSmartReports(){
  ...
  function petatoeMonthYearLabel(key){
    ...
    return `${smartReportT('calendar.months.'+MONTHS[mi],MAR[MONTHS[mi]]||parts[1])} ${yy}`;
  }
  ...
  const salesComparisonRows=salesComparisonKeys.map(k=>{
    ...
    return {
      key:k,
      previousKey:prevKey,
      label:[smartReportT('calendar.months.'+MONTHS[cur.monthIndex],MAR[MONTHS[cur.monthIndex]]||String(k)),String(cur.year||'')],
  ...
  const heatMonthLabel=(m,mi)=> heatmapActiveYear==='all' ? `${smartReportHtml('calendar.months.'+m,smartReportT('calendar.months.'+m,MAR[m]))}<small>${smartReportHtml('filters.allYears','كل السنوات')}</small>` : `${smartReportHtml('calendar.months.'+m,smartReportT('calendar.months.'+m,MAR[m]))}<small>${heatmapActiveYear}${isFutureHeatMonth(mi)?' - '+smartReportHtml('heatmap.noInvoices','لا توجد فواتير'):''}</small>`;
  ...
  smartSafeHTML('smartReportsArea',`
    <div class="smart-actions-row" id="smartTabs"><button class="smart-pill active" data-smart-tab="overview" data-smart-action="smart-tab" data-tab="overview">${smartReportHtml('tabs.performanceSummary','ملخص الأداء')}</button><button class="smart-pill" data-smart-tab="sales" data-smart-action="smart-tab" data-tab="sales">${smartReportHtml('tabs.salesAnalysis','تحليل المبيعات')}</button><button class="smart-pill" data-smart-tab="vehicles" data-smart-action="smart-tab" data-tab="vehicles">${smartReportHtml('tabs.vehicleAnalysis','تحليل السيارات')}</button><button class="smart-pill" data-smart-tab="customers" data-smart-action="smart-tab" data-tab="customers">${smartReportHtml('tabs.customerAnalysis','تحليل العملاء')}</button><button class="smart-pill" data-smart-tab="services" data-smart-action="smart-tab" data-tab="services">${smartReportHtml('tabs.serviceAnalysis','تحليل الخدمات')}</button><button class="smart-pill" data-smart-tab="advanced" data-smart-action="smart-tab" data-tab="advanced">${smartReportHtml('tabs.advancedReports','مركز التقارير المتقدمة')}</button><button class="smart-pill smart-pill-merged-ai" data-smart-tab="forecast" data-smart-action="smart-tab" data-tab="forecast">${smartReportHtml('tabs.forecasting','التوقعات وذكاء الأعمال')}</button><button class="smart-pill" data-smart-tab="recommendations" data-smart-action="smart-tab" data-tab="recommendations">${smartReportHtml('tabs.recommendations','التوصيات')}</button></div>
    <div class="smart-tab-section active" data-smart-section="overview">${overviewCardsYearFilter}<div class="smart-exec-grid">
      ...
      <div class="smart-exec-kpi" style="--accent:var(--blue)">
        <button class="kpi-info-btn" type="button" aria-label="تفاصيل أكفأ سيارة">🚐</button>
        <div class="kpi-tooltip"><div class="tip-head"><span class="tip-ico">🚐</span><span>أكفأ سيارة</span></div><div class="tip-desc">ترتيب السيارة حسب متوسط الإيراد لكل عملية خلال الفترة المختارة.</div><div class="tip-grid"><div class="tip-row"><span>السيارة</span><b>${cardBestEffVan[0]}</b></div><div class="tip-row"><span>الإيراد</span><b>${money(cardBestEffVan[1]||0)}</b></div><div class="tip-row"><span>العمليات</span><b>${fmt0(cardBestEffVan[2]||0)}</b></div><div class="tip-row"><span>متوسط العملية</span><b>${money(cardBestEffVan[3]||0)}</b></div></div></div>
        <span>أكفأ سيارة</span><b>${cardBestEffVan[0]}</b><small>${money(cardBestEffVan[3])} / عملية</small>
      </div>
    ...
    <div class="smart-dash-grid smart-overview-stacked">
      ...
      <div class="smart-panel" id="smartVehicleEfficiencyPanel"><div class="smart-panel-head-with-filters"><div><h3>🚐 تحليل كفاءة السيارات</h3></div>${smartVehicleEfficiencyFilterHtml()}</div><div class="smart-table-clean"><table><thead><tr><th>السيارة</th><th>عدد العمليات</th><th>الإيراد</th><th>متوسط العملية</th><th>المساهمة</th></tr></thead><tbody id="smartVehicleEfficiencyBody">${smartVehicleEfficiencyRowsHtml()}</tbody></table></div></div>
    </div>
    ...
    <div class="sales-intel-section">
      ...
      <div class="sales-intel-grid">
        <div class="sales-intel-panel"><div class="chart-head sales-intel-monthly-head" style="align-items:flex-start;gap:12px;flex-wrap:wrap"><div><h3>📈 ${smartReportHtml('sales.monthlySalesTrend','اتجاه المبيعات الشهري')}</h3></div>${salesIntelMonthlyFilterHtml()}<div class="report-control-group sales-intel-monthly-modes" style="margin-inline-start:auto">${salesIntelMonthlyMetricButtons()}</div></div><div class="sales-intel-chart tall"><canvas id="salesIntelMonthly"></canvas></div></div>
        <div class="sales-intel-panel"><div class="chart-head sales-intel-month-compare-head" style="align-items:flex-start;gap:12px;flex-wrap:wrap"><div><h3>📊 ${smartReportHtml('sales.monthToMonthComparison','مقارنة شهر بشهر')}</h3></div><div class="report-control-group sales-intel-month-compare-modes" style="margin-inline-start:auto">${salesIntelMonthCompareMetricButtons()}</div></div><div class="sales-intel-chart"><canvas id="salesIntelMonthCompare"></canvas></div><div class="sales-intel-table" style="margin-top:10px"><table><thead><tr><th>${smartReportHtml('sales.period','الفترة')}</th><th>${smartReportHtml('sales.current','الحالي')}</th><th>${smartReportHtml('sales.previous','السابق')}</th><th>${smartReportHtml('sales.difference','الفرق')}</th><th>%</th></tr></thead><tbody>${salesComparisonRows.map(r=>`<tr><td>${r.tableLabel}</td><td>${money(r.current)}</td><td>${money(r.previous)}</td><td>${money(r.diff)}</td><td class="${r.diff>=0?'metric-up':'metric-down'}" style="color:${r.diff>=0?'#22c55e':'#ef4444'};font-weight:900">${r.pct.toFixed(1)}%</td></tr>`).join('')}</tbody></table></div></div>
        <div class="sales-intel-panel">
          <h3>📅 ${smartReportHtml('sales.yearOverYearComparison','مقارنة سنة بسنة')}</h3>
          <p>${smartReportHtml('sales.monthlyVsYears','{base} مقابل {current} شهريًا.',{base:salesPrevYear,current:salesCurrentYear})}</p>
          <div class="yoy-control-panel">
            ...
            <div class="yoy-custom-row">
              <label class="yoy-check" data-smart-action="sales-yoy-custom-toggle">
                ...
                ${smartReportHtml('sales.customTwoYearComparison','مقارنة مخصصة بين سنتين')}
              </label>
              <div class="yoy-selects ${salesYoYCustomMode?'show':''}">
                <label>${smartReportHtml('sales.baseYear','سنة الأساس')}
                  <select data-smart-action="sales-yoy-base">${salesYoYOptions}</select>
                </label>
                <label>${smartReportHtml('sales.comparisonYear','سنة المقارنة')}
                  <select data-smart-action="sales-yoy-compare">${salesYoYOptions}</select>
                </label>
              </div>
            </div>
            <div class="yoy-active-badge">${smartReportHtml('sales.activeComparison','المقارنة النشطة: {base} ↔ {current}',{base:salesPrevYear,current:salesCurrentYear})}</div>
          </div>
  ...
  chart('salesIntelYoY',{type:'bar',data:{labels:MONTHS.map(m=>smartReportT('calendar.months.'+m,MAR[m])),datasets:[{label:String(salesPrevYear),data:salesYoYMonths.map(x=>x.prev),backgroundColor:'#94a3b8',borderRadius:8},{label:String(salesCurrentYear),data:salesYoYMonths.map(x=>x.cur),backgroundColor:css('--purple'),borderRadius:8}]},options:{...baseOpts(),layout:{padding:{top:28}},plugins:{...baseOpts().plugins,petatoeLabels:{enabled:true,money:true,font:'800 10px Cairo'}}}});
