...
function smartInteractionT(key, fallback, params){
  try{
    ...
    if(center&&typeof center.t==='function'){
      return center.t('smartReportsSource.interactions.'+key,params||{},{fallback:fallback,allowKeyFallback:true});
    }
  }catch(_){ }
...
function petatoeSmartHandleAction(el, ev){
  ...
  switch(action){
    ...
    case 'retry-sales-invoices': if(window.PETATOESmartTabs&&typeof window.PETATOESmartTabs.retrySalesInvoices==='function') window.PETATOESmartTabs.retrySalesInvoices(); return true;
