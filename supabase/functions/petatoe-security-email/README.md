...
## Phase S4.2 MFA Login Workflow

Additional supported actions:

```json
{"action":"mfa_send_otp","username":"i.alaa","email":"user@example.com","purpose":"mfa_email_otp"}
```

```json
{"action":"mfa_verify","username":"i.alaa","email":"user@example.com","otp":"123456","purpose":"mfa_email_otp"}
```

MFA OTP uses the same secure token table with `purpose = mfa_email_otp`, hashed OTP storage, expiry, single-use status, max attempts, and audit logging.
