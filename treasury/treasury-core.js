(function(){
  ...
  async function handover(){
    ...
    var tx={id:todayId('TR'),type:'handover',vehicle:v,amount:amt,receiver:rec,method:method,notes:notes,time:now()}; var saved=await saveTxToSupabase(tx); if(!saved.ok)return alert(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('فشل حفظ حركة الخزينة في Supabase: '):'فشل حفظ حركة الخزينة في Supabase: '+(saved.error||'')); var a=getTx();a.push(tx);setTx(a);addAudit('تسليم كاش',null,tx,'إنشاء حركة تسليم');clearForm();renderAll();toastMsg('تم تسليم الكاش وتحويله للخزنة الرئيسية');
  }
  async function expense(){
    ...
    var tx={id:todayId('EX'),type:'expense',source:src,vehicle:src===OWNER?'':src,category:cat,amount:amt,receiver:rec,ref:ref,notes:notes,time:now()}; var saved=await saveTxToSupabase(tx); if(!saved.ok)return alert(window.PETATOE_LOCALIZATION_CENTER&&window.PETATOE_LOCALIZATION_CENTER.translateRuntime?window.PETATOE_LOCALIZATION_CENTER.translateRuntime('فشل حفظ حركة الخزينة في Supabase: '):'فشل حفظ حركة الخزينة في Supabase: '+(saved.error||'')); var a=getTx();a.push(tx);setTx(a);var cc=getCats();cc.push(cat);setCats(cc);addAudit('صرف خزينة',null,tx,'إنشاء حركة صرف');clearExpenseForm();renderAll();toastMsg('تم تسجيل حركة الصرف');
  }
