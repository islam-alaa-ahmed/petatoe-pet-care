(function(){
  ...
  function cashRows(){return dataRows().filter(function(r){return rowVehicle(r) && isCash(rowPay(r))})}
  function cashByVehicle(){var m={};cashRows().forEach(function(r){var v=rowVehicle(r), a=rowAmount(r);m[v]=(m[v]||0)+a});return m}
  function vehicleDelivered(ignoreId){var m={};getTx().forEach(function(t){if(clean(t.id)===clean(ignoreId))return; if(t&&t.type==='handover'){var v=clean(t.vehicle);m[v]=(m[v]||0)+num(t.amount)}});return m}
  function expenseBySource(ignoreId){var m={};getTx().forEach(function(t){if(clean(t.id)===clean(ignoreId))return; if(t&&t.type==='expense'){var src=clean(t.source||t.vehicle||OWNER);m[src]=(m[src]||0)+num(t.amount)}});return m}
  ...
  function mainReceived(ignoreId){return getTx().reduce(function(s,t){return s+((t&&t.type==='handover'&&clean(t.id)!==clean(ignoreId))?num(t.amount):0)},0)}
  function mainSpent(ignoreId){return getTx().reduce(function(s,t){return s+((t&&t.type==='expense'&&clean(t.id)!==clean(ignoreId)&&clean(t.source||OWNER)===OWNER)?num(t.amount):0)},0)}
  ...
  function fillSelects(){
    ...
    setOptions(byId('trExpenseSource'),[OWNER].concat(vs),'',clean((byId('trExpenseSource')||{}).value)||OWNER);
    ensureStatementSelector();
    setOptions(byId('trStatementVaultSelectV82'),[OWNER].concat(vs),'اختر الخزنة لعرض كشف الحساب',activeStatementVault||clean((byId('trStatementVaultSelectV82')||{}).value));
  ...
  function renderKpis(){
    ...
    var todayHand=getTx().filter(function(t){return t.type==='handover'&&clean(t.time).slice(0,10)===today}).reduce(function(s,t){return s+num(t.amount)},0);
  ...
  function renderVaults(){
    ...
    if(owner)treasurySafeHtml(owner,'<div class="tr-owner-card"><button type="button" class="vico tr-statement-open" data-vault="'+esc(OWNER)+'" title="كشف حساب الخزنة الرئيسية">🏦</button><div class="tr-owner-content"><span class="tr-owner-title">الخزنة الرئيسية للمالك</span><div class="tr-owner-metrics"><div class="tr-owner-metric"><span>إجمالي المستلم</span><b>'+fmt(mainReceived())+'</b></div><div class="tr-owner-metric"><span>إجمالي المصروف</span><b>'+fmt(mainSpent())+'</b></div><div class="tr-owner-metric"><span>الرصيد الحالي</span><b>'+fmtAvail(mainBalance())+'</b></div></div></div></div>','treasury owner card');
  ...
  async function editTx(id){
    ...
    var rec=prompt('المسؤول / المستلم',clean(t.receiver)); if(rec===null)return; rec=clean(rec); if(!rec)return alert('المسؤول مطلوب');
  ...
  function ensureTabs(){
    ...
    if(!tabs){
      ...
      treasurySafeHtml(tabs,[['home','🏠 الواجهة الرئيسية'],['balances','💰 الأرصدة'],['movements','🤝 التسليمات'],['expenses','💸 المصروفات'],['statement','📜 كشف الحساب'],['log','📜 سجل الحركات'],['audit','🧾 سجل التدقيق'],['reports','📊 التقارير']].map(function(t){return '<button type="button" class="treasury-tab-btn-v82" data-tr-tab="'+t[0]+'">'+t[1]+'</button>'}).join(''),'treasury tabs');
  ...
  function openTreasuryTab(tab,silent){
    ...
    qa('#treasury .treasury-section-v82').forEach(function(s){s.classList.toggle('active',s.getAttribute('data-tr-section')===activeTreasuryTab)});
  ...
  function renderHome(home){
    ...
    treasurySafeHtml(old,'<div class="tr-home-card-v82" role="button" tabindex="0" data-tr-open-tab="balances"><b>💰 إجمالي أرصدة الخزن</b><span>'+fmtAvail(total+mainBalance())+' • عدد الخزن: '+(vs.length+1)+'</span></div><div class="tr-home-card-v82" role="button" tabindex="0" data-tr-open-tab="log"><b>🔄 الحركات المالية</b><span>عدد الحركات الحالية: '+rows+' • تحصيل / تسليم / صرف</span></div><div class="tr-home-card-v82" role="button" tabindex="0" data-tr-open-tab="statement"><b>📜 كشف الحساب</b><span>اختر أي خزنة واعرض الوارد والمنصرف والرصيد التفصيلي.</span></div>','treasury home cards');
  }
