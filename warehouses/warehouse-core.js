...
(function(){
  ...
  function exportMovementsCsv(){var header=[whT('date'),whT('movementType'),whT('item'),whT('from'),whT('to'),whT('quantity'),whT('responsible'),whT('reference'),whT('notes')]; downloadCsv('PETATOE_Warehouse_Movements.csv',[header].concat(filteredMovementRows().map(function(r){return[r.time,normalizeType(r.type),r.item,r.from,r.to,r.qty,r.person,r.ref,r.notes]})));}
...
(function(){
  ...
  function filteredRows(){
    ...
    return rowsAll().filter(function(r){
  ...
  function exportCsv(){var data=filteredRows(); var csv=[[whT('store'),whT('code'),whT('item'),whT('currentBalance'),whT('alertLimit'),whT('lastMovement')]].concat(data.map(function(r){return [r.store,r.code,r.item,r.balance,r.min,r.last||'']})).map(function(row){return row.map(function(c){return '"'+String(c==null?'':c).replace(/"/g,'""')+'"'}).join(',')}).join('\n'); var a=document.createElement('a'); a.href=URL.createObjectURL(new Blob(['\ufeff'+csv],{type:'text/csv;charset=utf-8'})); a.download='PETATOE_Warehouse_Low_Stock_Alerts.csv'; a.click(); setTimeout(function(){URL.revokeObjectURL(a.href)},700)}
  function bindEvents(){
